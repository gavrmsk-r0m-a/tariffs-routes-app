...
def create_demo_sqlite(output: str | Path) -> Path:
    ...
    try:
        ...
        q(conn, "INSERT INTO phone_number_history(phone_number_id, action, changed_by, changed_at, field_name, old_value, new_value, reason, comment) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)", (phone_id, "update", admin_id, STAGE46_PHONE_CHANGED_AT, "status", "unused", "used", "phone.changed", "Synthetic Stage 46 phone history"))
        q(conn, "INSERT INTO route_phone_number_history(route_id, phone_number_id, old_phone_number_id, new_phone_number_id, action, changed_by, changed_at, old_values, new_values, reason, comment) VALUES (?, ?, NULL, ?, ?, ?, ?, ?, ?, ?, ?)", (route_id, phone_id, phone_id, "attach", admin_id, STAGE46_ROUTE_PHONE_ADDED_AT, "", f"phone_number_id={phone_id}", "route.phone.added", "Synthetic Stage 46 route phone added"))
        q(conn, "INSERT INTO route_phone_number_history(route_id, phone_number_id, old_phone_number_id, new_phone_number_id, action, changed_by, changed_at, old_values, new_values, reason, comment) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)", (route_id, phone_id, routed_phone_id, phone_id, "replace", admin_id, STAGE46_ROUTE_PHONE_REPLACED_AT, f"phone_number_id={routed_phone_id}", f"phone_number_id={phone_id}", "route.phone.replaced", "Synthetic Stage 46 route phone replacement"))
        q(conn, "INSERT INTO route_history(route_id, action, changed_by, changed_at, field_name, old_value, new_value, reason, comment) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)", (route_id, "update", admin_id, STAGE46_ROUTE_CHANGED_AT, "comment", "Previous synthetic route", "Synthetic route", "route.changed", "Synthetic Stage 46 route history"))
        for changed_at, old_price, reason, comment in ((STAGE46_TARIFF_CREATED_AT, None, "tariff.created", "Synthetic Stage 46 tariff created"), (STAGE46_TARIFF_CHANGED_AT, "0.090000", "tariff.changed", "Synthetic Stage 46 tariff changed")):
            q(conn, """
                INSERT INTO tariff_change_history(tariff_id, changed_at, changed_by, country_id, country_name_snapshot,
                    provider_id, provider_name_snapshot, provider_prefix_id, prefix_snapshot, old_provider_currency_id,
                    new_provider_currency_id, old_price_in_provider_currency, new_price_in_provider_currency,
                    ...
                    new_conversion_rate_date, old_eur_price, new_eur_price, eur_price_delta, reason, comment, created_at)
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
            """, (tariff_id, changed_at, admin_id, country_id, "Demo Country", provider_id, "Demo Provider", prefix_id, "123", eur_id if old_price else None, eur_id, old_price, "0.100000", "1.000000" if old_price else None, "1.000000", TODAY if old_price else None, TODAY, "0.090000" if old_price else None, "0.100000", "0.010000" if old_price else "0.000000", reason, comment, changed_at))
