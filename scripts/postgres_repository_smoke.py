...
def run_stage_40_checks(repo: Repository, check) -> None:
    ...
    filters = [
        ...
        ("status_used", {"status": "used"}, {"525550000001", "525550000020"}), ("status_unused", {"status": "unused"}, {"525550000010"}), ("status_unknown", {"status": "unknown"}, set()), ("status_missing", {"status": "missing"}, set()),
    ]
    for name, filter_values, expected in filters:
    ...
    for value in ("1", 1, True):
        check(f"stage_40_review_required_true_{value!r}", lambda value=value: _check(numbers(repo.list_phone_numbers({"review_required": value})) == {"525550000010", "525550000020"}, "true review filter returned wrong phones"))
    for value in ("0", 0, False):
        check(f"stage_40_review_required_false_{value!r}", lambda value=value: _check(numbers(repo.list_phone_numbers({"review_required": value})) == {"525550000001"}, "false review filter returned wrong phones"))
    ...
    for value in ("1", 1, True):
        check(f"stage_40_is_problematic_true_{value!r}", lambda value=value: _check(numbers(repo.list_phone_numbers({"is_problematic": value})) == {"525550000010"}, "true problematic filter returned wrong phones"))
    for value in ("0", 0, False):
        check(f"stage_40_is_problematic_false_{value!r}", lambda value=value: _check(numbers(repo.list_phone_numbers({"is_problematic": value})) == {"525550000001", "525550000020"}, "false problematic filter returned wrong phones"))
    for name, value in (("all", "all"), ("empty", ""), ("none", None)):
        check(f"stage_40_is_problematic_{name}_ignored", lambda value=value: _check(numbers(repo.list_phone_numbers({"is_problematic": value})) == baseline, "ignored problematic filter must not restrict"))
    for value in ("true", "false", "yes", "invalid"):
        check(f"stage_40_is_problematic_invalid_{value}", lambda value=value: _check(repo.list_phone_numbers({"is_problematic": value}) == [], "invalid problematic filter must return []"))

    check("stage_40_combined_routed", lambda: _check(numbers(repo.list_phone_numbers({"country_id": routed["country_id"], "provider_id": routed["provider_id"], "project": "CI Phone Project", "project_like": "phone project", "assignment_type": "ivr", "status": "used", "number_like": "0000020", "review_required": "1", "is_problematic": "0"})) == {"525550000020"}, "combined routed filters must match exactly"))
    check("stage_40_combined_review", lambda: _check(numbers(repo.list_phone_numbers({"country_id": review["country_id"], "provider_id": 0, "project": "ИТМ", "assignment_type": "aon", "status": "unused", "number_like": "0000010", "review_required": "1", "is_problematic": "1"})) == {"525550000010"}, "combined review filters must match exactly"))
...
def run_stage_50_checks(repo: Repository, check) -> None:
    ...
    check("stage_50_route_pool_summary_semantics", lambda: _check(
        isinstance(route_summary, list)
        and all({
            "id", "name", "country_name", "provider_name", "total_count",
            "checked_count", "unchecked_count", "spam_count", "elevated_count",
            "low_count", "clean_count", "coverage",
        } <= set(row) for row in route_summary)
        and all(int(row["checked_count"]) + int(row["unchecked_count"]) == int(row["total_count"]) for row in route_summary)
        and all(int(row["clean_count"]) + int(row["low_count"]) + int(row["elevated_count"]) + int(row["spam_count"]) == int(row["checked_count"]) for row in route_summary)
        and all(0 <= float(row["coverage"]) <= 100 for row in route_summary),
        "SPAM route pool summary must expose internally consistent current-pool counts",
    ))
