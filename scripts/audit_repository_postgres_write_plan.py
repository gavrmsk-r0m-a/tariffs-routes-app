...
def audit(repository_file=ROOT/'app/repository.py',coverage_manifest=ROOT/'docs/postgres/repository_method_coverage.json',write_plan=ROOT/'docs/postgres/repository_write_surface_plan.json'):
 ...
 if isinstance(smoked,list):
  ...
  if set(smoked) != expected_smoked: errors.append('rollback_smoke_covered_methods must contain exactly the Stage 51-66D methods')
  ...
  for name in ('update_calling_company_import_fields','create_calling_company','update_calling_company','create_currency_rate','log_currency_rate_change','recalculate_current_tariffs_for_currency_rate'):
   if name in smoked: errors.append(f'{name} must remain pending after Stage 66C')
