...
def audit(repository_file=ROOT/'app/repository.py',coverage_manifest=ROOT/'docs/postgres/repository_method_coverage.json',write_plan=ROOT/'docs/postgres/repository_write_surface_plan.json'):
 ...
 if isinstance(smoked,list):
  ...
  expected_smoked={'set_hlr_limit_override','set_app_setting_value','delete_app_setting_value','upsert_hlr_daily_usage','create_user','update_user','update_user_password','set_user_permissions','create_country','create_currency','create_provider','create_prefix','get_or_create_country','get_or_create_currency','get_or_create_provider','get_or_create_prefix','ensure_project_exists','ensure_phone_number_type_exists','ensure_phone_assignment_type_exists','create_server','create_change_reason','update_dictionary_snapshots','update_server_route_priority','create_provider_change','deactivate_routing_event','update_routing_event','create_company_routing_setting','update_company_routing_setting','deactivate_company_routing_setting','create_routing_event','create_route','update_route','update_route_import_fields','create_phone_number','update_phone_number','record_phone_update_history','update_phone_number_import_fields_with_history'}
  if set(smoked) != expected_smoked: errors.append('rollback_smoke_covered_methods must contain exactly the Stage 51-66B methods')
  ...
  for name in ('update_calling_company_import_fields','create_calling_company','update_calling_company','add_phone_to_route','add_phone_to_route_by_number','remove_phone_links_from_route','create_tariff','update_tariff','set_tariff_active','create_currency_rate','log_currency_rate_change','recalculate_current_tariffs_for_currency_rate'):
   if name in smoked: errors.append(f'{name} must remain pending after Stage 66B')
