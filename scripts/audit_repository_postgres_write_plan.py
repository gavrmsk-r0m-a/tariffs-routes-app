...
BASELINE={"repository_public_methods_count":124,"smoke_covered_read_count":70,"deferred_read_only_count":0,"write_or_mutating_count":53,"infrastructure_or_mixed_count":1,"read_surface_coverage_percent":100.0,"repository_smoke_checks_count":615}
...
def audit(repository_file=ROOT/'app/repository.py',coverage_manifest=ROOT/'docs/postgres/repository_method_coverage.json',write_plan=ROOT/'docs/postgres/repository_write_surface_plan.json'):
 ...
 if not isinstance(expected,dict) or len(expected)!=BASELINE['write_or_mutating_count']: raise ConfigError('coverage write_or_mutating is invalid')
 ...
 if isinstance(smoked,list):
  ...
  expected_smoked={'set_hlr_limit_override','set_app_setting_value','delete_app_setting_value','upsert_hlr_daily_usage','create_user','update_user','update_user_password','set_user_permissions','clear_user_permissions','create_country','create_currency','create_provider','create_prefix','get_or_create_country','get_or_create_currency','get_or_create_provider','get_or_create_prefix','ensure_project_exists','ensure_phone_number_type_exists','ensure_phone_assignment_type_exists','create_server','create_change_reason','update_change_reason','update_dictionary_snapshots','update_server_route_priority','create_provider_change','deactivate_routing_event','update_routing_event','create_company_routing_setting','update_company_routing_setting','deactivate_company_routing_setting','create_routing_event','create_route','update_route','update_route_import_fields','create_phone_number','update_phone_number','record_phone_update_history','update_phone_number_import_fields_with_history','add_phone_to_route','add_phone_to_route_by_number','remove_phone_links_from_route','create_tariff','update_tariff','set_tariff_active','create_currency_rate','log_currency_rate_change','recalculate_current_tariffs_for_currency_rate','create_calling_company','update_calling_company','update_company_routing_setting_comment','update_calling_company_import_fields','save_spam_check_batch'}
  if set(smoked) != expected_smoked: errors.append('rollback_smoke_covered_methods must contain exactly the Stage 51-66F methods')
  if any(name in smoked for name in ('_route_prefix_id','_current_tariff','_change_log','_server_route_priority_summary','_sync_company_routing_comment_from_event','_active_company_routing_setting')): errors.append('private helpers must not be rollback-smoked')
  ...
  if 'create_routing_event' not in smoked or methods.get('create_routing_event',{}).get('batch')!='routing_event_application_writes': errors.append('create_routing_event must be a Stage 65B routing-event rollback probe')
  for name, batch in (('create_route','phone_route_tariff_core_writes'),('update_route','phone_route_tariff_core_writes'),('update_route_import_fields','importer_and_bulk_mutation_writes')):
   meta=methods.get(name,{})
   if name not in smoked or meta.get('batch') != batch or meta.get('current_commit_behavior') != 'mixed' or meta.get('transaction_contract') != 'optional_commit_parameter' or 'Stage 66A route import rollback smoke' not in meta.get('notes',''): errors.append(f'{name} must be a Stage 66A route-import rollback probe')
  for name, batch in (('create_phone_number','phone_route_tariff_core_writes'),('update_phone_number','phone_route_tariff_core_writes'),('record_phone_update_history','phone_route_tariff_core_writes'),('update_phone_number_import_fields_with_history','importer_and_bulk_mutation_writes')):
   meta=methods.get(name,{})
   if name not in smoked or meta.get('batch') != batch or meta.get('current_commit_behavior') != 'mixed' or meta.get('transaction_contract') != 'optional_commit_parameter' or 'Stage 66B phone import rollback smoke' not in meta.get('notes',''): errors.append(f'{name} must be a Stage 66B phone-import rollback probe')
  for name in ('add_phone_to_route','add_phone_to_route_by_number','remove_phone_links_from_route'):
   meta=methods.get(name,{})
   if name not in smoked or meta.get('batch') != 'phone_route_tariff_core_writes' or meta.get('current_commit_behavior') != 'mixed' or meta.get('transaction_contract') != 'optional_commit_parameter' or 'Stage 66C route-phone link rollback smoke' not in meta.get('notes',''): errors.append(f'{name} must be a Stage 66C route-phone rollback probe')
  for name in ('create_tariff','update_tariff','set_tariff_active'):
   meta=methods.get(name,{})
   if name not in smoked or meta.get('batch') != 'phone_route_tariff_core_writes' or meta.get('current_commit_behavior') != 'mixed' or meta.get('transaction_contract') != 'optional_commit_parameter' or 'Stage 66D tariff lifecycle rollback smoke' not in meta.get('notes',''): errors.append(f'{name} must be a Stage 66D tariff lifecycle rollback probe')
  for name in ('create_currency_rate','log_currency_rate_change','recalculate_current_tariffs_for_currency_rate'):
   meta=methods.get(name,{})
   expected_behavior = 'mixed' if name == 'create_currency_rate' else 'never commits'
   expected_contract = 'optional_commit_parameter' if name == 'create_currency_rate' else 'multi_statement_atomic'
   if name not in smoked or meta.get('batch') != 'phone_route_tariff_core_writes' or meta.get('current_commit_behavior') != expected_behavior or meta.get('transaction_contract') != expected_contract or 'Stage 66E currency-rate rollback smoke' not in meta.get('notes',''): errors.append(f'{name} must be a Stage 66E currency-rate rollback probe')
  create_event_notes=methods.get('create_routing_event',{}).get('notes','')
  if create_event_notes != 'Stage 64 covered none/server_priority. Stage 65B adds campaign_setting and completes rollback-only coverage for create_routing_event.': errors.append('create_routing_event must retain the Stage 65B completion note')
  routing_methods={name for name, meta in methods.items() if isinstance(meta, dict) and meta.get('batch') == 'routing_event_application_writes'}
  if routing_methods-set(smoked): errors.append('routing_event_application_writes methods must all be rollback-smoked')
  for name in ('create_company_routing_setting','update_company_routing_setting','deactivate_company_routing_setting'):
   if name not in smoked or methods.get(name,{}).get('batch')!='company_routing_setting_writes': errors.append(f'{name} must be a Stage 65A company-routing rollback probe')
  for name, batch in (('create_calling_company','company_routing_setting_writes'),('update_calling_company','company_routing_setting_writes'),('update_company_routing_setting_comment','company_routing_setting_writes'),('update_calling_company_import_fields','importer_and_bulk_mutation_writes')):
   meta=methods.get(name,{})
   if name not in smoked or meta.get('batch') != batch or meta.get('current_commit_behavior') != 'mixed' or meta.get('transaction_contract') != 'optional_commit_parameter' or 'Stage 66F' not in meta.get('notes',''): errors.append(f'{name} must be a Stage 66F calling-company rollback probe')
  if writes-set(smoked): errors.append('all write_or_mutating methods must be rollback-smoked after Stage 66F')
  ...
  for name in ('set_app_setting_value','delete_app_setting_value','upsert_hlr_daily_usage','create_user','update_user','update_user_password','set_user_permissions','clear_user_permissions'):
