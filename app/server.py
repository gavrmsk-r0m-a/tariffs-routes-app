...
<head>
  ...
  <style>
    ...
    * {{ box-sizing: border-box; }}
    ...
    .friendly-validation {{ display: flex; align-items: flex-start; gap: 10px; border: 1px solid #d99a2b; background: color-mix(in srgb, #f59e0b 12%, var(--surface)); color: var(--text); padding: 12px 14px; border-radius: var(--radius-card); line-height: 1.45; }}
    .friendly-validation .material-symbols-rounded {{ flex: 0 0 auto; color: #b76e00; font-size: 20px; margin-top: 1px; }}
    ...
    .modal-blocking-error {{ position: fixed; z-index: 1100; top: max(12px, env(safe-area-inset-top)); left: 50%; width: min(680px, calc(100vw - 24px)); box-sizing: border-box; transform: translateX(-50%); display: flex; align-items: flex-start; gap: 10px; padding: 12px 14px; border: 1px solid #991b1b; border-radius: 12px; background: #c81e1e; color: #fff; box-shadow: 0 12px 30px rgba(69, 10, 10, .34); font-weight: 720; line-height: 1.4; white-space: normal; overflow-wrap: anywhere; }}
    .modal-blocking-error .material-symbols-rounded {{ flex: 0 0 auto; margin-top: 1px; }}
    .modal-blocking-error strong {{ display: block; color: #fff; }}
    .modal-blocking-error [data-blocking-error-message] {{ display: block; font-weight: 620; }}
    .has-blocking-error, input.has-blocking-error, select.has-blocking-error, textarea.has-blocking-error {{ border-color: #dc2626 !important; outline: 2px solid rgba(220, 38, 38, .22); outline-offset: 1px; }}
    ...
    .phones-page .filter-card .filter-grid .filter-review-checkbox input[type="checkbox"] {{ flex: 0 0 16px; width: 16px; height: 16px; min-width: 16px; min-height: 16px; margin: 0; padding: 0; }}
...
<body>
  ...
  <script>
    ...
    function clearModalBlockingError() {{
      document.querySelectorAll(".modal-blocking-error").forEach((node) => node.remove());
    }}
    modalDetails.forEach((details) => {{
      const form = details.querySelector("form");
      enhanceModalForm(form, () => {{ details.removeAttribute("open"); clearModalBlockingError(); }});
      details.addEventListener("toggle", () => {{ if (!details.open) clearModalBlockingError(); }});
    ...
    adminEditDetails.forEach((details) => {{
      ...
      if (dedicatedCancel) {{
        ...
        actions.appendChild(cancel);
        cancel.addEventListener("click", () => {{ details.removeAttribute("open"); clearModalBlockingError(); }});
      }}
      details.addEventListener("toggle", () => {{
        if (details.open) closeAdminEdit(details);
        else clearModalBlockingError();
      }});
    }});
...
def _spam_risk(value: int) -> tuple[str, str]:
    value = max(0, min(5, int(value)))
    if value == 0: return "clean", "Чисто"
    if value <= 2: return "low", "Низкий риск"
    if value <= 4: return "elevated", "Повышенный риск"
    return "danger", "SPAM"


def _spam_score(value: int) -> str:
    value = max(0, min(5, int(value)))
    level, label = _spam_risk(value)
    return f"<span class='spam-score spam-score-{level}' title='{label}'><b>{'■'*value}{'□'*(5-value)}</b> {value}/5 {'SPAM' if value == 5 else ''}</span>"


def _spam_prepare_preview(repo: Repository, result: dict) -> dict:
    """Attach purchased-number existence and saveability to a parser preview."""
    candidates = [row.number for row in result["rows"] if row.status not in {"extra", "missing"}]
    states = repo.spam_states(candidates)
    for row in result["rows"]:
        if row.status == "ready" and row.number not in states:
            row.status = "not_found"
    summary = result["summary"]
    summary["not_found"] = sum(row.status == "not_found" for row in result["rows"])
    summary["errors"] = sum(row.status == "error" for row in result["rows"]) + len(result["issues"])
    summary["saveable"] = sum(row.status == "ready" for row in result["rows"])
...
def spam_checker_page(repo: Repository, q: dict[str, str] | None = None, *, data: dict | None = None,
    ...
    if tab == "checked":
        checked_view = q.get("checked_view", "numbers")
        secondary = "".join(f"<a class='spam-subtab {'active' if checked_view == key else ''}' href='/spam-checker?tab=checked&checked_view={key}'>{label}</a>" for key, label in (("numbers", "Номера"), ("routes", "Маршруты")))
        route_rows = repo.spam_route_pool_summary({k:q.get(k,"") for k in ("country_id","provider_id","source_type","spam_only","unchecked_only")})
        if checked_view == "routes":
            if q.get("export") == "csv":
                return csv_response("spam_routes.csv", ["Route","GEO","Provider","AON type","Total numbers","Checked","Unchecked","Spam 5","Risk 3-4","Risk 1-2","Clean 0","Last pool check"], [[r.get("name"),r.get("country_name"),r.get("provider_name"),r.get("cli_source_type"),r.get("total_count"),r.get("checked_count"),r.get("unchecked_count"),r.get("spam_count"),r.get("elevated_count"),r.get("low_count"),r.get("clean_count"),r.get("last_checked_at")] for r in route_rows])
            route_filter=f"<form class='spam-filter' method='get'><input type='hidden' name='tab' value='checked'><input type='hidden' name='checked_view' value='routes'><label>ГЕО<select name='country_id'>{opts(countries,q.get('country_id'))}</select></label><label>Провайдер<select name='provider_id'>{opts(providers,q.get('provider_id'))}</select></label><label>Тип АОН<select name='source_type'><option value='all'>Все</option><option value='pool' {'selected' if q.get('source_type')=='pool' else ''}>Pool</option><option value='sim' {'selected' if q.get('source_type')=='sim' else ''}>SIM</option></select></label><label class='spam-only'><input type='checkbox' name='spam_only' value='1' {'checked' if q.get('spam_only') else ''}> Только маршруты со SPAM</label><label class='spam-only'><input type='checkbox' name='unchecked_only' value='1' {'checked' if q.get('unchecked_only') else ''}> Только с непроверенными</label><div class='spam-filter-actions'><button>Найти</button><a class='button secondary' href='/spam-checker?{esc(urlencode({**q,'tab':'checked','checked_view':'routes','export':'csv'}))}'>Экспорт CSV</a></div></form>"
            ...
            if q.get("export")=="csv":
                return csv_response("spam_checked.csv",["Number","GEO","Provider","Project","Assignment","Current routes","Last check","Last verdict","Source","Checks count","Score","Risk status","SPAM yes/no"],[[r.get("number"),r.get("country_name"),r.get("provider_name"),r.get("project_label"),r.get("assignment_label"),r.get("current_route_names"),r.get("last_checked_at"),r.get("last_verdict"),r.get("last_source"),r.get("checks_count"),r.get("score"),_spam_risk(r.get("score",0))[1],"yes" if r.get("score")==5 else "no"] for r in rows])
            route_options="<option value=''>Все</option>"+"".join(f"<option value='{r['id']}' {'selected' if str(q.get('route_id',''))==str(r['id']) else ''}>{esc(r['name'])}</option>" for r in route_rows)
            risk_options="".join(f"<option value='{key}' {'selected' if q.get('risk_status')==key else ''}>{label}</option>" for key,label in (("","Все"),("clean","Чисто"),("low","Низкий риск"),("elevated","Повышенный риск"),("spam","SPAM")))
            stale_options="<option value=''>Не применять</option>"+"".join(f"<option value='{d}' {'selected' if q.get('stale_days')==str(d) else ''}>{d} дней</option>" for d in (7,14,30,60,90))
            filter_form=f"<form class='spam-filter' method='get'><input type='hidden' name='tab' value='checked'><input type='hidden' name='checked_view' value='numbers'><label>Номер<input name='number' value='{esc(q.get('number',''))}'></label><label>ГЕО<select name='country_id'>{opts(countries,q.get('country_id'))}</select></label><label>Провайдер<select name='provider_id'>{opts(providers,q.get('provider_id'))}</select></label><label>Проект<input name='project' value='{esc(q.get('project',''))}'></label><label>Назначение<input name='assignment' value='{esc(q.get('assignment',''))}'></label><label>Текущий маршрут<select name='route_id'>{route_options}</select></label><label>Рейтинг<select name='score'><option value=''>Все</option>{''.join(f'<option value={i} {"selected" if q.get("score")==str(i) else ""}>{i}</option>' for i in range(6))}</select></label><label>Статус риска<select name='risk_status'>{risk_options}</select></label><label>Источник<input name='source' value='{esc(q.get('source',''))}'></label><label>Последняя проверка<input type='date' name='last_date' value='{esc(q.get('last_date',''))}'></label><label>Не проверялись более<select name='stale_days'>{stale_options}</select></label><label class='spam-only'><input type='checkbox' name='spam_only' value='1' {'checked' if q.get('spam_only') else ''}> Только SPAM 5/5</label><label class='spam-only'><input type='checkbox' name='elevated_only' value='1' {'checked' if q.get('elevated_only') else ''}> Только 3–4</label><div class='spam-filter-actions'><button>Найти</button><a class='button secondary' href='/spam-checker?{esc(urlencode({**q,'tab':'checked','checked_view':'numbers','export':'csv'}))}'>Экспорт CSV</a></div></form>"
            keys=("number","country_id","provider_id","project","assignment","route_id","score","risk_status","source","last_date","stale_days","spam_only","elevated_only")
            filters_html=filter_card(filter_form,q,keys).replace("href='/spam-checker?reset_filters=1'", "href='/spam-checker?tab=checked&checked_view=numbers'")
            counts={"spam":0,"elevated":0,"low":0,"clean":0,"stale":0}
            now=datetime.now()
            for r in rows:
                level,_=_spam_risk(r.get('score',0)); counts[level if level!='danger' else 'spam']+=1
                try: counts['stale'] += (now-datetime.fromisoformat(str(r.get('last_checked_at')))).days>30
                except (ValueError,TypeError): pass
            summary="".join(f"<div><strong>{value}</strong><span>{label}</span></div>" for label,value in (("Проверено номеров",len(rows)),("SPAM 5/5",counts['spam']),("Риск 3–4",counts['elevated']),("Низкий риск 1–2",counts['low']),("Чистые 0",counts['clean']),("Не проверялись >30 дней",counts['stale'])))
            body_rows=[]
            for r in rows:
                level,label=_spam_risk(r.get('score',0)); retry=urlencode({"tab":"phones","numbers":r['number']})
                body_rows.append(f"<tr><td>{esc(r['number'])}<br><a class='small-action' href='/spam-checker?{retry}'>Проверить снова</a></td><td>{esc(r.get('country_name'))}</td><td>{esc(r.get('provider_name'))}</td><td>{esc(r.get('project_label'))}</td><td>{esc(r.get('assignment_label'))}</td><td>{esc(r.get('current_route_names') or '—')}</td><td>{esc(r.get('last_checked_at'))}</td><td>{esc(r.get('last_verdict'))}</td><td>{esc(r.get('last_source') or '—')}</td><td>{r.get('checks_count')}</td><td>{_spam_score(r.get('score',0))}</td><td><span class='risk-badge risk-{level}'>{label}</span></td><td><a href='/spam-checker/{r['id']}/history'>История</a></td></tr>")
            content=f"<div class='spam-content spam-checked'><nav class='spam-subtabs'>{secondary}</nav>{filters_html}<section class='spam-summary' aria-label='Сводка по текущим фильтрам'>{summary}</section><div class='table-wrap spam-checked-table'><table><tr><th>Номер</th><th>ГЕО</th><th>Провайдер</th><th>Проект</th><th>Назначение</th><th>Текущие маршруты</th><th>Последняя проверка</th><th>Последний результат</th><th>Источник</th><th>Проверок</th><th>Рейтинг</th><th>Статус</th><th>История</th></tr>{''.join(body_rows)}</table></div></div>"
    else:
        ...
        if tab=="phones":
            selected_option = lambda value, current, label: f"<option value='{value}' {'selected' if str(current or '') == value else ''}>{label}</option>"
            boolean_options = lambda current: "<option value=''>Все</option>" + selected_option("1", current, "Да") + selected_option("0", current, "Нет")
            left = (
                f"<label>ГЕО<select name='country_id'>{opts(countries,country)}</select></label>"
                f"<label>Провайдер<select name='provider_id'>{opts(providers,provider)}</select></label>"
                f"<label>Проект<select name='project'>{project_options(repo, selected=data.get('project') or None, empty='Все')}</select></label>"
                f"<label>Назначение<select name='assignment_type'>{assignment_options(repo, selected=data.get('assignment_type') or None, empty='Все')}</select></label>"
                f"<label>Тип номера<select name='phone_type'>{phone_type_options(repo, selected=data.get('phone_type') or None, empty='Все')}</select></label>"
                f"<label>Рабочий статус<select name='status'>{phone_status_options(data.get('status') or None, empty='Все')}</select></label>"
                f"<label>Активен у провайдера<select name='is_active'>{boolean_options(data.get('is_active'))}</select></label>"
                f"<label>Проблемный<select name='is_problematic'>{boolean_options(data.get('is_problematic'))}</select></label>"
                f"<label>Требует проверки<select name='review_required'>{boolean_options(data.get('review_required'))}</select></label>"
            )
        else:
            routes=repo.spam_eligible_routes(int(country) if str(country).isdigit() else None,source_type)
            route_list="".join(f"<label class='route-choice'><input type='checkbox' name='route_{r['id']}' value='1' {'checked' if data.get('route_'+str(r['id'])) or q.get('route_'+str(r['id'])) else ''} {'disabled' if int(r['phone_count'])==0 else ''}><span>{esc(r['name'])}<small>{esc(r['provider_name'])} · {r['phone_count']} номеров</small></span></label>" for r in routes)
            selected_routes=sum(bool(data.get('route_'+str(r['id'])) or q.get('route_'+str(r['id']))) for r in routes)
            left=f"<div class='spam-route-basics'><label>ГЕО<select name='country_id'>{opts(countries,country)}</select></label><label>Тип АОН<select name='source_type'><option value='pool' {'selected' if source_type=='pool' else ''}>Pool</option><option value='sim' {'selected' if source_type=='sim' else ''}>SIM</option></select></label></div><label class='route-selector-label'>Маршруты</label><details class='route-selector'><summary><span class='route-placeholder'>Выберите маршруты</span><span class='route-selected'>Выбрано: {selected_routes}</span></summary><div class='route-panel'><div class='route-tools'><button class='button secondary' type='button' data-route-toggle='all'>Выбрать все</button><button class='button secondary' type='button' data-route-toggle='none'>Снять все</button></div><div class='route-list'>{route_list or '<p class="muted">Подходящих маршрутов нет</p>'}</div></div></details>"
        numbers=data.get("numbers",q.get("numbers","")); raw=data.get("raw_response",""); token=data.get("request_token",str(uuid.uuid4())); route_map=data.get("route_map","{}")
        errors=data.get("errors",[]); error_html=f"<div class='flash error'>{'<br>'.join(esc(x) for x in errors)}</div>" if errors else ""
        preview_html=""
        if preview:
            states=preview.get("states") or repo.spam_states([r.number for r in preview["rows"] if r.status not in {"extra", "missing"}])
            summary=preview["summary"]
            trs=[]
            route_obj=json.loads(route_map or "{}")
            for r in preview["rows"]:
                before=states.get(r.number,0); delta,after,_=score_change(before,r.verdict,r.sources) if r.status=="ready" else (None,None,"")
                status={"ready":"Готово","missing":"Не сохранится · Нет результата от SPAM Checker","extra":"Не сохранится · Лишний номер в ответе","error":"Не сохранится · Конфликт результатов","not_found":"Не сохранится · Номер отсутствует в «Купленных номерах»"}[r.status]
                if r.duplicate: status += " · Повтор в ответе"
                trs.append(f"<tr><td>{esc(r.number)}</td><td>{esc(', '.join(x['name'] for x in route_obj.get(r.number,[])) or '—')}</td><td>{esc(r.verdict)}</td><td>{esc(', '.join(r.sources) or '—')}</td><td>{_spam_score(before) if r.number in states else '—'}</td><td>{f'{delta:+d}' if delta is not None else '—'}</td><td>{_spam_score(after) if after is not None else '—'}</td><td>{esc(status)}</td></tr>")
            preview_html=f"<section class='card spam-preview'><h2>Предпросмотр</h2><p>Запрошено: {summary['requested']} · Получено: {summary['received']} · Spam: {summary['spam']} · Clear: {summary['clear']} · Нет результата: {summary['missing']} · Лишних: {summary['extra']} · К сохранению: {summary.get('saveable',0)} · Не найдено: {summary.get('not_found',0)} · Ошибок: {summary['errors']}</p><div class='table-wrap'><table><tr><th>Номер</th><th>Маршрут(ы)</th><th>Результат</th><th>Источник</th><th>Текущий рейтинг</th><th>Δ</th><th>Новый рейтинг</th><th>Статус разбора</th></tr>{''.join(trs)}</table></div><button type='submit' name='action' value='save' formaction='/spam-checker/save' formmethod='post'{' disabled aria-disabled="true"' if not summary.get('saveable') else ''}>Сохранить результаты</button></section>"
        mode_class=" spam-work-phones" if tab=="phones" else " spam-work-routes"
        content=f"""<form class='spam-content' method='post' action='/spam-checker'>{error_html}<input type='hidden' name='selection_mode' value='{tab}'><input type='hidden' name='request_token' value='{esc(token)}'><input type='hidden' name='route_map' value='{esc(route_map)}'><section class='card spam-work spam-selection{mode_class}'><div class='spam-left'><h2>Фильтры / выбор</h2>{left}</div><div class='spam-numbers'><h2>Номера для проверки</h2><textarea id='spam-numbers' name='numbers' rows='10'>{esc(numbers)}</textarea></div><footer class='spam-actions'><button type='submit' name='action' value='generate' formaction='/spam-checker/generate' formmethod='post'>Сформировать список</button><button class='secondary' type='button' onclick="document.getElementById('spam-numbers').value=''">Очистить</button></footer></section><section class='card spam-result'><h2>Результат SPAM Checker</h2><p class='muted'>Вставьте полный ответ DG_spam_bot. Строка с номером без пометки считается чистой. Hiya добавляет +1 к рейтингу, любая другая пометка — +2. Заголовки spam / clear не обязательны. Сначала нажмите «Разобрать результат» — это не изменит данные.</p><textarea name='raw_response' rows='10'>{esc(raw)}</textarea><button type='submit' name='action' value='preview' formaction='/spam-checker/preview' formmethod='post'>Разобрать результат</button></section>{preview_html}</form>"""
...
def handle_spam_checker_post(repo: Repository, parsed: dict[str, str], action: str, checked_by: int) -> tuple[str, bytes]:
    ...
    if not numbers and mode == "phones":
        # Parser V2 remains the single authority for recognizing phone lines.
        inferred = parse_response(raw_response, [])
        numbers = [row.number for row in inferred["rows"]]
    if not numbers:
        errors.append("Нет номеров для проверки.")
    result = _spam_prepare_preview(repo, parse_response(raw_response, numbers)) if numbers else None
    ...
    if result:
        errors.extend(result["issues"])
        saveable = [row for row in result["rows"] if row.status == "ready"]
        if not saveable:
            errors.append("Нет корректных результатов для сохранения.")
        if saveable and not result["issues"]:
            try:
                route_map = json.loads(parsed.get("route_map") or "{}")
            except (ValueError, TypeError):
                route_map = {}
            prepared = [{"number": row.number, "verdict": row.verdict, "sources": row.sources, "source": row.source, "status": row.status} for row in saveable]
            saved = repo.save_spam_check_batch(request_token=parsed.get("request_token", ""), selection_mode=mode, raw_response=raw_response, expected_numbers=numbers, results=prepared, route_map=route_map, checked_by=checked_by, parser_version=PARSER_VERSION)
            skipped = len(numbers) - saved["saved"]
            if saved["duplicate"]:
                message = f"Результаты этой проверки уже были сохранены: {saved['saved']} из {len(numbers)}."
            else:
                message = f"Сохранено {saved['saved']} из {len(numbers)}." + (f" Пропущено {skipped}." if skipped else "")
...
def modal_blocking_error(message: str | None) -> str:
    """Render one fixed, accessible error for a user-correctable modal save."""
    if not message:
        return ""
    return f"""<div class='modal-blocking-error' role='alert' aria-live='assertive'>
<span class='material-symbols-rounded' aria-hidden='true'>error</span>
<span><strong>Не получилось сохранить.</strong><span data-blocking-error-message>{esc(message)}</span></span>
</div>"""
...
def bulk_phones_page(repo: Repository, form_data: dict[str, str] | None = None, *, result: BulkPhoneResult | None = None, form_error: str | None = None) -> bytes:
    ...
    notification = ""
    if form_error:
        notification = f"<div class='bulk-phone-notification' role='alert'><strong>Не получилось сохранить.</strong><br>{esc(form_error)}</div>"
    elif result and result.action == "save":
        if result.saved and result.failed:
            notification = f"<div class='bulk-phone-notification is-partial' role='status'>Готово. Сохранено: {result.saved}, ошибок: {result.failed}.</div>"
        elif result.saved:
            notification = f"<div class='bulk-phone-notification is-success' role='status'>Готово. Сохранено: {result.saved} номеров.</div>"
        else:
            notification = f"<div class='bulk-phone-notification' role='alert'>Не удалось сохранить номера. Ошибок: {result.failed}.</div>"
...
def dictionaries_page(repo: Repository, q: dict[str, str] | None = None, *, form_error: str | None = None, form_data: dict[str, str] | None = None) -> bytes:
    ...
    error_fields = {"countries": "name", "providers": "name", "currencies": "code", "prefixes": "prefix", "servers": "name", "phone-types": "name", "projects": "name", "phone-assignments": "name"}

    def mark_problem_control(control_html: str, field: str, *, editing: bool) -> str:
        is_target_form = ("_entity_id" in form_data) == editing
        if not form_error or form_error == DICTIONARY_CONFIRMATION_ERROR or not is_target_form or error_fields[active_section] != field:
            return control_html
        return re.sub(rf"<(input|select|textarea)\b(?=[^>]*\bname=['\"]{re.escape(field)}['\"])", r"<\1 class='has-blocking-error' aria-invalid='true' autofocus", control_html, count=1)
    ...
    def edit_field(label: str, control_html: str, field: str | None = None) -> str:
        if field:
...
<div class='notice warning dictionary-confirmation-area{' has-blocking-error' if confirmation_error else ''}'><strong>Preview массового обновления:</strong><ul>{count_items}</ul><label class='dictionary-mass-confirm'><input type='checkbox' name='confirm_update_linked' value='1' {'checked' if confirmation_checked else ''} {'aria-invalid="true" autofocus' if confirmation_error else ''}><span>Подтверждаю обновление связанных записей, если выбран массовый режим.</span></label></div>
</fieldset>"""
    ...
    def add_form(section: str) -> str:
        ...
        if section == "providers":
            return f"<form class='form-grid' method='post' action='/admin/dictionaries/providers/create'><label>Провайдер <input name='name' value='{value('name')}' placeholder='Название провайдера' required></label><label>Валюта <select name='default_currency_id' required><option value=''>—</option>{options(repo, 'currencies', 'code', selected=form_data.get('default_currency_id'))}</select></label><label>Комментарий <input name='comment' value='{value('comment')}' placeholder='Комментарий'></label>{submit}</form>"
        ...
        if section == "prefixes":
            return f"<form class='form-grid' method='post' action='/admin/dictionaries/prefixes/create'><label>Провайдер <select name='provider_id' required><option value=''>—</option>{options(repo, 'providers', selected=form_data.get('provider_id'))}</select></label><label>Префикс <input name='prefix' value='{value('prefix')}' placeholder='0827' required></label><label>Комментарий <input name='name' value='{value('name')}' placeholder='Комментарий'></label>{submit}</form>"
        if section == "servers":
        ...
        if section == "phone-types":
            return f"<form class='form-grid' method='post' action='/admin/dictionaries/phone-types/create'><label>Тип номера <input name='name' value='{value('name')}' placeholder='Mobile' required></label><label>Комментарий <input name='comment' value='{value('comment')}' placeholder='Комментарий'></label>{submit}</form>"
        if section == "projects":
...
<div class='dictionary-layout'>
  ...
  <section class='dictionary-workspace'>
    ...
    {modal_blocking_error(form_error)}
    <details class='dictionary-add'{' open' if form_error and '_entity_id' not in form_data else ''}><summary>+ Добавить значение</summary>{mark_problem_control(add_form(active_section), error_fields[active_section], editing=False)}</details>
    {table_card(table_html)}
...
DICTIONARY_DUPLICATE_MESSAGES = {
    "countries": "Кажется, такое GEO у нас уже есть. Давай назовём его иначе.",
    "providers": "Кажется, такой провайдер у нас уже есть. Давай назовём его иначе.",
    "currencies": "Кажется, такая валюта у нас уже есть. Давай используем другой код.",
    "prefixes": "Кажется, такой префикс у этого провайдера уже есть. Давай укажем другой.",
    "servers": "Кажется, такой сервер у нас уже есть. Давай назовём его иначе.",
    "phone-types": "Кажется, такой тип номера у нас уже есть. Давай назовём его иначе.",
    "projects": "Кажется, такой проект у нас уже есть. Давай назовём его иначе.",
    "phone-assignments": "Кажется, такое назначение у нас уже есть. Давай назовём его иначе.",
}


def ensure_dictionary_value_unique(repo: Repository, kind: str, value: str, *, entity_id: int | None = None, provider_id: int | None = None, column: str | None = None) -> None:
    """Apply the user-facing normalized guard while retaining DB UNIQUE fallback."""
    tables = {
        "countries": ("countries", "name"), "currencies": ("currencies", "code"),
        "servers": ("servers", "name"), "phone-types": ("phone_number_types", "name"),
        "projects": ("projects", "name"), "phone-assignments": ("phone_assignment_types", column or "name"),
    }
    p = placeholder(repo.backend)
    if kind == "providers":
        sql, params = f"SELECT id FROM providers WHERE normalized_name = {p}", [normalize_provider_name(value)]
    elif kind == "prefixes":
        sql, params = f"SELECT id FROM provider_prefixes WHERE provider_id = {p} AND prefix = {p}", [provider_id, value]
    else:
        table, field = tables[kind]
        sql, params = f"SELECT id FROM {table} WHERE LOWER(TRIM({field})) = {p}", [value.strip().lower()]
    if entity_id is not None:
        sql += f" AND id <> {p}"
        params.append(entity_id)
    if repo.conn.execute(sql + " LIMIT 1", tuple(params)).fetchone():
        if kind == "phone-assignments" and column == "code":
            raise BusinessRuleError("Кажется, такой код назначения у нас уже есть. Давай используем другой.")
        raise BusinessRuleError(DICTIONARY_DUPLICATE_MESSAGES[kind])
...
def handle_post(repo: Repository, path: str, data: dict[str, str]):
    ...
    if path.startswith("/admin/dictionaries/") and path.endswith("/create"):
        ...
        if kind == "countries":
            ...
            name = required_dictionary_text(data, "name", "Кажется, мы забыли название провайдера. Давай его укажем.")
            ensure_dictionary_value_unique(repo, kind, name)
            ...
            code = required_dictionary_text(data, "code", "Кажется, мы забыли код валюты. Давай его укажем.").upper()
            ensure_dictionary_value_unique(repo, kind, code)
            ...
            ensure_dictionary_value_unique(repo, kind, prefix, provider_id=provider_id)
            repo.create_prefix(provider_id, prefix, name)
        elif kind == "servers":
            name = required_dictionary_text(data, "name", "Кажется, мы забыли название сервера. Давай его укажем.")
            ensure_dictionary_value_unique(repo, kind, name)
            comment = (data.get("comment") or "").strip() or None
...
def user_error(exc: Exception) -> str:
    ...
    if db_error.kind == UNIQUE_VIOLATION:
        ...
        dictionary_duplicates = (
            ("provider_prefixes", DICTIONARY_DUPLICATE_MESSAGES["prefixes"]),
            ("phone_number_types", DICTIONARY_DUPLICATE_MESSAGES["phone-types"]),
            ("currencies", DICTIONARY_DUPLICATE_MESSAGES["currencies"]),
            ("countries", DICTIONARY_DUPLICATE_MESSAGES["countries"]),
            ("providers", DICTIONARY_DUPLICATE_MESSAGES["providers"]),
            ("servers", DICTIONARY_DUPLICATE_MESSAGES["servers"]),
            ("projects", DICTIONARY_DUPLICATE_MESSAGES["projects"]),
        )
        if "phone_assignment_types" in metadata:
            return "Кажется, такой код назначения у нас уже есть. Давай используем другой." if "code" in columns or "code" in metadata else DICTIONARY_DUPLICATE_MESSAGES["phone-assignments"]
...
def app(environ, start_response):
    ...
    try:
            ...
            if path == "/phones/bulk-create":
                try:
                    ...
                    if action not in {"validate", "save"}:
                        raise BusinessRuleError("Некорректное действие массового добавления.")
                    result = process_bulk_phones(repo, parsed, current_actor_id(), save=action == "save")
                    start_response("200 OK", html_headers())
        ...
        is_dictionary_request = path.startswith("/admin/dictionaries/")
        if not isinstance(exc, BusinessRuleError) and not (isinstance(exc, ValueError) and not is_dictionary_request):
            db_error = map_database_error(exc, backend=DB_CONFIG.backend)
