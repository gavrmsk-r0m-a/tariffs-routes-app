...
def spam_checker_page(repo: Repository, q: dict[str, str] | None = None, *, data: dict | None = None,
    ...
    if tab == "checked":
        ...
        content=f"<div class='spam-content spam-checked'>{filters_html}<div class='table-wrap spam-checked-table'><table><tr><th>Номер</th><th>ГЕО</th><th>Провайдер</th><th>Проект</th><th>Назначение</th><th>Маршруты</th><th>Последняя проверка</th><th>Результат</th><th>Источник</th><th>Проверок</th><th>Рейтинг</th><th>История</th></tr>{body_rows}</table></div></div>"
    else:
        ...
        if tab=="phones":
            ...
            selected_routes=sum(bool(data.get('route_'+str(r['id']))) for r in routes)
