...
<form method="post" action="/admin/company-routing-settings/create">
  ...
  <label>Режим маршрутизации <span class="required">*</span><select name="routing_mode" id="create_routing_mode">{routing_mode_options(q.get('routing_mode') or 'server_priority')}</select></label>
  <label>Маршрут кампании <select name="route_id">{route_options_for_country(repo, create_country_id)}</select></label>
  <label>Авторотация <input type="checkbox" name="has_autorotation" value="1" id="create_has_autorotation"></label>
  <label>Активна <input type="checkbox" name="is_active" value="1" checked></label>
  <label>Комментарий <input name="comment"></label>
...
<script>
const createRoutingMode = document.getElementById('create_routing_mode');
const createHasAutorotation = document.getElementById('create_has_autorotation');
if (createRoutingMode && createHasAutorotation) {{
  createRoutingMode.addEventListener('change', () => {{
    if (createRoutingMode.value === 'autorotation') createHasAutorotation.checked = true;
  }});
}}
</script>
"""
