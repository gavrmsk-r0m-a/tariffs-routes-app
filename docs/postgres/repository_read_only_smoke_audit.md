# Repository read-only PostgreSQL smoke audit (Stage 35)
...
| `get_phone_number`, `get_route`, `route_numbers` | Phone/route details and route relation lookup. | compatible | Backend placeholders and parameterized active booleans added | added in Stage 35 | IDs come only from the existing phone identity and calling-company Repository results; no direct smoke SQL is used. |
...
All eight Stage 35 methods execute only `SELECT`, do not call `commit`,
...
Still deferred are `_user_columns`, `list_users`, `get_user`, and
`get_user_by_username` together with their PRAGMA/information-schema and
`COLLATE NOCASE` design decisions; `query_filters` and `search_text_matches`;
