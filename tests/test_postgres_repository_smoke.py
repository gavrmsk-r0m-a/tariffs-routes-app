...
class PostgreSQLRepositorySmokeTest(unittest.TestCase):
    ...
    def test_stage_36_methods_are_in_smoke_plan(self):
        self.assertEqual(("list_users", "get_user", "get_user_by_username", "authenticate_user", "count_active_admins"), smoke.STAGE_36_METHODS)
        self.assertTrue(set(smoke.STAGE_36_METHODS) <= set(smoke.SMOKE_METHODS))
        self.assertEqual(1, smoke.SMOKE_METHODS.count("count_active_admins"))
    ...
    def test_stage_46_methods_are_declared_once(self):
        self.assertEqual(("list_phone_history", "list_route_history", "list_tariff_history"), smoke.STAGE_46_METHODS)
        for method in smoke.STAGE_46_METHODS:
            self.assertEqual(1, smoke.SMOKE_METHODS.count(method))
        self.assertTrue(set(smoke.STAGE_46_METHODS) <= set(smoke.SMOKE_METHODS))

    def test_stage_47_methods_are_declared_once(self):
        self.assertEqual(("list_company_routing_setting_history",), smoke.STAGE_47_METHODS)
        self.assertEqual(1, smoke.SMOKE_METHODS.count("list_company_routing_setting_history"))

    def test_stage_48_methods_are_declared_once(self):
        self.assertEqual(("list_calling_company_history",), smoke.STAGE_48_METHODS)
        self.assertEqual(1, smoke.SMOKE_METHODS.count("list_calling_company_history"))

    def test_stage_49_methods_are_declared_once(self):
        self.assertEqual(("list_calling_company_events", "count_calling_company_events"), smoke.STAGE_49_METHODS)
    ...
    def test_stage_49_failure_continues_checks(self):
        ...
        original = repository.repository.list_calling_company_events
        def wrong_events(*args, **kwargs):
            rows = list(original(*args, **kwargs))
            return [{**dict(row), "company_id_external": "wrong"} if row["comment"] == "Stage 49 routing beta" else row for row in rows]
        repository.repository.list_calling_company_events = wrong_events
        summary = self.run_demo(repository)
        self.assertEqual("failed", summary["status"])
        self.assertIn("stage_49_routing_contract", {failure["check"] for failure in summary["failures"]})
        self.assertGreater(repository.called.count("list_calling_company_events"), 3)
        self.assertGreater(repository.called.count("count_calling_company_events"), 3)
        self.assertTrue(set(smoke.STAGE_49_METHODS) <= set(repository.called))
        self.assertFalse([name for name in repository.called if name.startswith(("create_", "update_", "ensure_", "delete_", "set_", "upsert_"))])
        self.assertEqual(CHECKS_COUNT, summary["checks_count"])

    def test_stage_48_failure_continues_checks(self):
        repository = RecordingRepository(Repository(self.conn))
        original = repository.repository.list_calling_company_history
        def wrong_history(company_id):
            rows = original(company_id)
            if rows and any(row["comment"] == "Stage 48 routing-event history" for row in rows):
                rows = list(rows)
                for index, row in enumerate(rows):
                    if row["comment"] == "Stage 48 routing-event history":
                        rows[index] = {**dict(row), "new_value": {"calling_company_id": company_id, "routing_mode": "wrong", "stage": 48}}
            return rows
        repository.repository.list_calling_company_history = wrong_history
        summary = self.run_demo(repository)
        self.assertEqual("failed", summary["status"])
        self.assertIn("stage_48_routing_values", {failure["check"] for failure in summary["failures"]})
        self.assertGreaterEqual(repository.called.count("list_calling_company_history"), 3)
    ...
    def test_stage_47_failure_continues_checks(self):
        repository = RecordingRepository(Repository(self.conn))
        original = repository.repository.list_company_routing_setting_history
        def wrong_history(setting_id):
            rows = original(setting_id)
            if rows and any(row["reason"] == "Stage 47 demo campaign active" for row in rows):
                rows = list(rows)
                for index, row in enumerate(rows):
                    if row["reason"] == "Stage 47 demo campaign active":
                        rows[index] = {**dict(row), "new_company_has_autorotation": 1}
            return rows
        repository.repository.list_company_routing_setting_history = wrong_history
        summary = self.run_demo(repository)
        self.assertEqual("failed", summary["status"])
    ...
    def test_stage_46_failure_continues_and_keeps_exact_check_count(self):
        repository = RecordingRepository(Repository(self.conn))
        original = repository.repository.list_tariff_history

        def wrong_tariff_history(tariff_id):
            rows = original(tariff_id)
            return [{**dict(row), "eur_price_delta": Decimal("999")} if row["reason"] == "tariff.changed" else row for row in rows]

        repository.repository.list_tariff_history = wrong_tariff_history
        summary = self.run_demo(repository)
        self.assertEqual("failed", summary["status"])
        self.assertIn("stage_46_tariff_history_changed", {failure["check"] for failure in summary["failures"]})
        self.assertIn("get_tariff", repository.called)
        self.assertTrue({"list_phone_history", "list_route_history", "list_tariff_history"} <= set(repository.called))
        write_prefixes = ("create_", "update_", "ensure_", "delete_", "clear_", "set_", "upsert_", "add_", "remove_", "recalculate_", "log_")
        self.assertFalse([name for name in repository.called if name.startswith(write_prefixes)])
        self.assertEqual(CHECKS_COUNT, summary["checks_count"])
