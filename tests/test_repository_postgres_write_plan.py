...
class WritePlanTests(unittest.TestCase):
 ...
 def test_actual_baseline_write_plan_passes(self):
  summary=self.execute_plan(self.plan); self.assertEqual('ok',summary['status']); self.assertEqual(19,summary['rollback_smoke_covered_methods_count'])
 ...
 def test_missing_stage56_rollback_method_fails(self):
  p=copy.deepcopy(self.plan); p['rollback_smoke_covered_methods'].remove('ensure_phone_number_type_exists'); self.bad(p)
