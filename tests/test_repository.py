...
class RepositoryBusinessRulesTest(unittest.TestCase):
    ...
    def test_create_phone_number_stores_native_booleans(self):
        row = self.conn.execute(
            'SELECT is_active, review_required FROM phone_numbers WHERE id = %s', (self.create_full_phone(),),
        ).fetchone()
    ...
    def test_create_phone_number_history_payload_preserved(self):
        phone_id = self.create_full_phone()
        row = self.conn.execute('SELECT * FROM phone_number_history WHERE phone_number_id = %s', (phone_id,)).fetchone()
        self.assertEqual(
    ...
    def test_create_phone_number_change_log_payload_preserved(self):
        phone_id = self.create_full_phone()
        row = self.conn.execute(
            "SELECT * FROM change_log WHERE entity_type = 'phone_number' AND entity_id = %s", (phone_id,),
        ).fetchone()
    ...
    def test_create_phone_number_commit_false(self):
        ...
        self.assertEqual(self.conn.execute(
            "SELECT COUNT(*) FROM change_log WHERE entity_type = 'phone_number' AND entity_id = %s", (phone_id,),
        ).fetchone()[0], 1)
        self.conn.rollback()
        self.assertIsNone(self.conn.execute('SELECT id FROM phone_numbers WHERE id = %s', (phone_id,)).fetchone())

    def test_create_phone_number_uses_postgres_returning_id(self):
    ...
    def test_update_phone_number_import_fields_with_history_stores_native_booleans(self):
        ...
        row = self.conn.execute('SELECT is_active, review_required FROM phone_numbers WHERE id = %s', (phone_id,)).fetchone()
        self.assertEqual((row["is_active"], row["review_required"]), (0, 1))
        self.assertTrue(all(isinstance(row[key], int) for key in ("is_active", "review_required")))
    ...
    def test_update_phone_number_import_fields_with_history_inserts_history_once(self):
        ...
        self.update_phone_import(phone_id)
        after = self.conn.execute('SELECT COUNT(*) FROM phone_number_history WHERE phone_number_id = %s', (phone_id,)).fetchone()[0]
        self.assertEqual(after, before + 1)
    ...
    def test_update_phone_number_import_fields_with_history_history_payload_preserved(self):
        ...
        row = self.conn.execute('SELECT action, field_name, changed_by, old_value, new_value, comment FROM phone_number_history WHERE phone_number_id = %s ORDER BY id DESC', (phone_id,)).fetchone()
        self.assertEqual(tuple(row.values()), ("updated", "import", self.admin_id, None, "new detail", "comment detail"))
    ...
    def test_repository_new_lookup_methods(self):
        self.conn.execute("INSERT INTO projects(name, is_active) VALUES ('Lookup Project', FALSE)")
        self.conn.execute("INSERT INTO phone_number_types(name, is_active) VALUES ('Lookup Type', TRUE)")
        self.conn.execute("INSERT INTO phone_assignment_types(code, name, is_active) VALUES ('lookup_code', 'Lookup Assignment', FALSE)")
    ...
    def test_repository_importer_exists_methods_return_bools(self):
        ...
        phone_identity = self.repo.get_phone_number_import_identity_by_normalized_number("393331239099")
        self.assertEqual(phone_identity["id"], phone_id)
        self.assertIn("imported_created_by", phone_identity)
        self.assertIsNone(self.repo.get_phone_number_import_identity_by_normalized_number("393331239098"))
    ...
    def test_update_route_import_fields_updates_only_import_fields_and_preserves_relations(self):
        ...
        self.conn.execute(
            "INSERT INTO route_phone_numbers(route_id, phone_number_id, usage_type, added_by, comment) VALUES (%s, %s, 'pool_member', %s, 'keep link')",
            (self.route_id, phone_id, self.admin_id),
        )
        server_id = self.repo.create_server("route-import-server")
        self.conn.execute(
            "INSERT INTO server_route_priorities(country_id, server_id, current_route_id, has_overflow, created_by, comment) VALUES (%s, %s, %s, FALSE, %s, 'keep priority')",
            (self.country_id, server_id, self.route_id, self.admin_id),
        )
        self.conn.commit()
        before = dict(self.conn.execute('SELECT * FROM routes WHERE id = %s', (self.route_id,)).fetchone())
        history_count = self.conn.execute('SELECT COUNT(*) FROM route_history WHERE route_id = %s', (self.route_id,)).fetchone()[0]
        ...
        after = dict(self.conn.execute('SELECT * FROM routes WHERE id = %s', (self.route_id,)).fetchone())
        ...
        self.assertEqual(self.conn.execute('SELECT comment FROM route_phone_numbers WHERE route_id = %s', (self.route_id,)).fetchone()[0], "keep link")
        self.assertEqual(self.conn.execute('SELECT comment FROM server_route_priorities WHERE current_route_id = %s', (self.route_id,)).fetchone()[0], "keep priority")
        self.assertEqual(self.conn.execute('SELECT COUNT(*) FROM route_history WHERE route_id = %s', (self.route_id,)).fetchone()[0], history_count)
    ...
    def test_ensure_project_exists_creates_missing_project(self):
        ...
        row = self.conn.execute('SELECT name, is_active FROM projects WHERE name = %s', ("Stage 23 Project",)).fetchone()
        self.assertEqual(rowcount, 1)
    ...
    def test_ensure_project_exists_is_idempotent(self):
        ...
        self.assertEqual(rowcount, 0)
        self.assertEqual(self.conn.execute('SELECT COUNT(*) FROM projects WHERE name = %s', ("Stage 23 Project",)).fetchone()[0], 1)
    ...
    def test_ensure_phone_number_type_exists_creates_missing_type(self):
        ...
        row = self.conn.execute('SELECT name, is_active FROM phone_number_types WHERE name = %s', ("Stage 23 Mobile",)).fetchone()
        self.assertEqual(rowcount, 1)
    ...
    def test_ensure_phone_number_type_exists_is_idempotent(self):
        ...
        self.assertEqual(rowcount, 0)
        self.assertEqual(self.conn.execute('SELECT COUNT(*) FROM phone_number_types WHERE name = %s', ("Stage 23 Mobile",)).fetchone()[0], 1)
    ...
    def test_ensure_phone_assignment_type_exists_creates_missing_assignment_type(self):
        ...
        row = self.conn.execute('SELECT code, name, is_active FROM phone_assignment_types WHERE code = %s', ("stage24",)).fetchone()
        self.assertEqual(rowcount, 1)
    ...
    def test_ensure_phone_assignment_type_exists_is_idempotent(self):
        ...
        self.assertEqual(rowcount, 0)
        self.assertEqual(self.conn.execute('SELECT COUNT(*) FROM phone_assignment_types WHERE code = %s', ("stage24",)).fetchone()[0], 1)
    ...
    def test_ensure_phone_assignment_type_exists_defaults_name_to_code(self):
        ...
        row = self.conn.execute('SELECT code, name, is_active FROM phone_assignment_types WHERE code = %s', ("stage24_default",)).fetchone()
        self.assertEqual(rowcount, 1)
    ...
    def test_set_user_permissions_inserts_permissions(self):
        ...
        row = self.conn.execute(
            'SELECT can_read, can_write, can_export FROM user_permissions WHERE user_id = %s AND section_key = %s',
            (user_id, "routes"),
        ).fetchone()
    ...
    def test_repository_transaction_commits_on_success(self):
        ...
        row = self.conn.execute(
            'SELECT id, rate_to_eur FROM currency_rates WHERE id = %s',
            (rate_id,),
        ).fetchone()
        self.assertIsNotNone(row)
    ...
    def test_repository_transaction_rolls_back_on_error(self):
        ...
        count = self.conn.execute(
            'SELECT COUNT(*) AS count FROM currency_rates WHERE currency_id = %s',
            (usd_id,),
        ).fetchone()["count"]
    ...
    def test_create_currency_rate_is_append_only(self):
        ...
        rows = self.conn.execute(
            'SELECT id, rate_to_eur FROM currency_rates WHERE currency_id = %s ORDER BY id',
            (usd_id,),
        ).fetchall()
    ...
    def test_currency_rate_recalculation_skips_inactive_tariffs(self):
        ...
        before = self.conn.execute('SELECT * FROM tariffs WHERE id = %s', (tariff_id,)).fetchone()
        new_rate_id = self.repo.create_currency_rate(usdt_id, "300", "2026-07-11", self.admin_id, commit=False)

        recalculated = self.repo.recalculate_current_tariffs_for_currency_rate(new_rate_id, self.admin_id)

        self.assertEqual(recalculated, [])
        after = self.conn.execute('SELECT * FROM tariffs WHERE id = %s', (tariff_id,)).fetchone()
    ...
    def test_dictionary_rename_without_updating_linked_records_keeps_phone_snapshots(self):
        ...
        self.conn.execute('UPDATE providers SET name = %s, normalized_name = %s WHERE id = %s', ("Miatel New", "miatel new", self.provider_id))
        assignment_id = self.conn.execute("SELECT id FROM phone_assignment_types WHERE code = 'gl'").fetchone()["id"]
        self.conn.execute('UPDATE phone_assignment_types SET name = %s WHERE id = %s', ("ГЛ NEW", assignment_id))
        self.conn.commit()
    ...
    def test_dictionary_rename_with_updating_linked_records_refreshes_phone_snapshots(self):
        ...
        old = self.conn.execute('SELECT name FROM providers WHERE id = %s', (self.provider_id,)).fetchone()["name"]
        self.conn.execute('UPDATE providers SET name = %s, normalized_name = %s WHERE id = %s', ("Miatel Fixed", "miatel fixed", self.provider_id))
    ...
    def test_inactive_dictionary_rename_does_not_reactivate_value(self):
        ...
        self.conn.execute('UPDATE projects SET is_active = FALSE WHERE id = %s', (project_id,))
        self.conn.commit()
        ...
        self.conn.execute('UPDATE projects SET name = %s, is_active = FALSE WHERE id = %s', ("ИТМ legacy", project_id))
        self.conn.commit()

        phone = self.repo.list_phone_numbers({"number_like": "393331234573"})[0]
        project = self.conn.execute('SELECT is_active FROM projects WHERE id = %s', (project_id,)).fetchone()
    ...
    def test_routing_event_created_with_updated_at(self):
        ...
        updated_at = self.conn.execute('SELECT updated_at FROM routing_events WHERE id = %s', (event_id,)).fetchone()["updated_at"]
        self.assertTrue(updated_at)

    def test_routing_event_update_changes_updated_at(self):
        event_id = self._create_basic_routing_event()
        original = self.conn.execute('SELECT updated_at FROM routing_events WHERE id = %s', (event_id,)).fetchone()["updated_at"]
        self.repo.update_routing_event(event_id, comment="updated", updated_at_original=original, updated_by=self.admin_id)
        row = self.conn.execute('SELECT comment, updated_at FROM routing_events WHERE id = %s', (event_id,)).fetchone()
    ...
    def test_routing_event_stale_updated_at_does_not_save(self):
        ...
        row = self.conn.execute('SELECT comment FROM routing_events WHERE id = %s', (event_id,)).fetchone()
    ...
    def test_reference_defaults_seed_current_projects_and_assignments(self):
        ...
        assignments = [tuple(row.values()) for row in self.conn.execute(
    ...
    def test_tariff_update_and_activation_history(self):
        ...
        row = self.conn.execute('SELECT reason, comment FROM tariff_change_history WHERE tariff_id = %s ORDER BY id DESC LIMIT 1', (tariff_id,)).fetchone()
        ...
        history_count = self.conn.execute('SELECT COUNT(*) FROM tariff_change_history WHERE tariff_id = %s', (tariff_id,)).fetchone()[0]
        self.assertFalse(self.repo.update_tariff(
            tariff_id, provider_currency_id=usd_id, price_in_provider_currency="3.1",
            conversion_rate_to_eur="0.9", conversion_rate_date="2026-06-22", currency_rate_id=None,
            comment="new terms", updated_by=self.admin_id,
        ))
        self.assertEqual(self.conn.execute('SELECT COUNT(*) FROM tariff_change_history WHERE tariff_id = %s', (tariff_id,)).fetchone()[0], history_count)
    ...
    def test_tariff_update_succeeds_with_current_token(self):
        tariff_id = self._create_tariff("2.5")
        token = self.conn.execute('SELECT updated_at FROM tariffs WHERE id = %s', (tariff_id,)).fetchone()["updated_at"]
    ...
    def test_tariff_update_rejects_stale_token(self):
        tariff_id = self._create_tariff("2.5")
        stale_token = self.conn.execute('SELECT updated_at FROM tariffs WHERE id = %s', (tariff_id,)).fetchone()["updated_at"]
        self.repo.update_tariff(
    ...
    def test_tariff_update_stale_token_does_not_write_history_or_change_log(self):
        ...
        before_history = self.conn.execute('SELECT COUNT(*) FROM tariff_change_history WHERE tariff_id = %s', (tariff_id,)).fetchone()[0]
        before_log = self.conn.execute("SELECT COUNT(*) FROM change_log WHERE entity_type = 'tariff' AND entity_id = %s", (tariff_id,)).fetchone()[0]
        ...
        self.assertEqual(self.conn.execute('SELECT COUNT(*) FROM tariff_change_history WHERE tariff_id = %s', (tariff_id,)).fetchone()[0], before_history)
        self.assertEqual(self.conn.execute("SELECT COUNT(*) FROM change_log WHERE entity_type = 'tariff' AND entity_id = %s", (tariff_id,)).fetchone()[0], before_log)
    ...
    def test_user_password_is_hashed_and_salted(self):
        ...
        row = self.conn.execute('SELECT password_hash, password_salt FROM users WHERE id = %s', (user_id,)).fetchone()
    ...
    def test_route_numbers_only_lists_currently_usable_provider_active_numbers(self):
        ...
        for index, status in enumerate(visible_statuses):
            ...
            self.conn.execute(
                "INSERT INTO route_phone_numbers(route_id, phone_number_id, usage_type, is_active, added_by) VALUES (%s, %s, 'pool_member', TRUE, %s)",
                (self.route_id, phone_id, self.admin_id),
            )
        ...
        self.conn.execute('UPDATE phone_numbers SET is_active = FALSE WHERE id = %s', (inactive_id,))
        self.conn.commit()
    ...
    def test_deactivating_phone_closes_route_links_and_logs_history_and_change_log(self):
        ...
        link = self.conn.execute('SELECT is_active, removed_at, removed_by FROM route_phone_numbers WHERE id = %s', (link_id,)).fetchone()
        ...
        history = self.conn.execute(
            "SELECT * FROM route_phone_number_history WHERE route_id = %s AND phone_number_id = %s AND action = 'removed'",
            (self.route_id, phone_id),
        ).fetchone()
        self.assertIsNotNone(history)
        log = self.conn.execute(
            "SELECT * FROM change_log WHERE entity_type = 'route_phone_number' AND entity_id = %s AND change_type = 'route_phone_number.removed_by_phone_deactivation'",
            (link_id,),
        ).fetchone()
    ...
    def test_phone_update_history_records_readable_field_changes(self):
        ...
        row = self.conn.execute(
            "SELECT action, old_value, new_value FROM phone_number_history WHERE phone_number_id = %s AND action = 'updated' ORDER BY id DESC",
            (phone_id,),
        ).fetchone()
        self.assertIsNotNone(row)
    ...
    def test_route_update_succeeds_with_current_token(self):
        token = self.conn.execute('SELECT updated_at FROM routes WHERE id = %s', (self.route_id,)).fetchone()["updated_at"]
        ...
        updated = self.conn.execute('SELECT name, comment FROM routes WHERE id = %s', (self.route_id,)).fetchone()
    ...
    def test_route_update_rejects_stale_token(self):
        stale_token = self.conn.execute('SELECT updated_at FROM routes WHERE id = %s', (self.route_id,)).fetchone()["updated_at"]
        self.repo.update_route(
            self.route_id,
        ...
        current = self.conn.execute('SELECT name, comment FROM routes WHERE id = %s', (self.route_id,)).fetchone()
    ...
    def test_route_update_stale_token_does_not_write_history(self):
        stale_token = self.conn.execute('SELECT updated_at FROM routes WHERE id = %s', (self.route_id,)).fetchone()["updated_at"]
        before_count = self.conn.execute('SELECT COUNT(*) AS count FROM route_history WHERE route_id = %s', (self.route_id,)).fetchone()["count"]
        self.repo.update_route(
            self.route_id,
        ...
        after_fresh_count = self.conn.execute('SELECT COUNT(*) AS count FROM route_history WHERE route_id = %s', (self.route_id,)).fetchone()["count"]
        ...
        after_stale_count = self.conn.execute('SELECT COUNT(*) AS count FROM route_history WHERE route_id = %s', (self.route_id,)).fetchone()["count"]
    ...
    def test_route_update_without_token_preserves_existing_internal_behavior(self):
        ...
        updated = self.conn.execute('SELECT name, comment, is_actual, priority_status FROM routes WHERE id = %s', (self.route_id,)).fetchone()
    ...
    def test_route_phone_add_remove_history_is_not_duplicated_by_field_history(self):
        ...
        events = self.conn.execute(
            'SELECT action, COUNT(*) AS count FROM route_phone_number_history WHERE route_id = %s AND phone_number_id = %s GROUP BY action',
            (self.route_id, phone_id),
        ).fetchall()
    ...
    def test_reactivation_review_can_be_cleared_without_repeating_history(self):
        ...
        row = self.conn.execute('SELECT review_required FROM phone_numbers WHERE id = %s', (phone_id,)).fetchone()
    ...
    def test_past_deactivation_does_not_force_review_after_manual_clear(self):
        ...
        row = self.conn.execute('SELECT review_required, deactivated_at FROM phone_numbers WHERE id = %s', (phone_id,)).fetchone()
        self.assertEqual(row["review_required"], 0)
        self.assertIsNotNone(row["deactivated_at"])
        details = self._latest_phone_update_details(phone_id)
    ...
    def test_calling_company_name_is_required_normalized_and_atomic(self):
        ...
        self.assertEqual(self.conn.execute('SELECT COUNT(*) FROM calling_companies WHERE server_id = %s', (server_id,)).fetchone()[0], 0)
    ...
    def test_calling_company_blank_update_preserves_row_and_history(self):
        ...
        history_count = self.conn.execute("SELECT COUNT(*) FROM change_log WHERE entity_type = 'calling_company' AND entity_id = %s", (company_id,)).fetchone()[0]
        ...
        self.assertEqual(self.conn.execute("SELECT COUNT(*) FROM change_log WHERE entity_type = 'calling_company' AND entity_id = %s", (company_id,)).fetchone()[0], history_count)
    ...
    def test_multi_geo_calling_company_initial_autorotation_creates_nullable_setting(self):
        ...
        row = self.conn.execute('SELECT * FROM company_routing_settings WHERE calling_company_id = %s', (company_id,)).fetchone()
    ...
    def test_multi_geo_mixed_and_autorotation_off_preserve_route_state(self):
        ...
        mixed = self.conn.execute('SELECT * FROM company_routing_settings WHERE id = %s', (setting_id,)).fetchone()
        ...
        campaign_route = self.conn.execute('SELECT route_id, routing_mode, has_autorotation FROM company_routing_settings WHERE id = %s', (campaign_route_id,)).fetchone()
        ...
        autorotation = self.conn.execute('SELECT id FROM company_routing_settings WHERE calling_company_id = %s', (second_company,)).fetchone()
        ...
        server_priority = self.conn.execute('SELECT route_id, routing_mode, has_autorotation FROM company_routing_settings WHERE id = %s', (server_priority_id,)).fetchone()
    ...
    def create_priority(self, current_route_id: int | None = None, previous_route_id: int | None = None) -> tuple[int, int]:
        ...
        self.conn.commit()
        return int(cur.fetchone()["id"]), server_id
    ...
    def test_calling_company_creation_autorotation_creates_routing_source_of_truth(self):
        ...
        setting = self.conn.execute('SELECT * FROM company_routing_settings WHERE calling_company_id = %s AND is_active = TRUE AND valid_to IS NULL', (company_id,)).fetchone()
    ...
    def test_calling_company_list_autorotation_ignores_stale_company_field(self):
        ...
        active = self.conn.execute('SELECT id FROM company_routing_settings WHERE calling_company_id = %s AND is_active = TRUE AND valid_to IS NULL', (company_id,)).fetchone()
    ...
    def test_manual_server_priority_route_change_moves_current_to_previous_and_logs_event(self):
        ...
        row = self.conn.execute('SELECT * FROM server_route_priorities WHERE id = %s', (priority_id,)).fetchone()
        ...
        event = self.conn.execute(
            "SELECT * FROM change_log WHERE entity_type = 'server_route_priority' AND entity_id = %s AND change_type = 'server_route_priority.current_route_updated'",
            (priority_id,),
        ).fetchone()
    ...
    def create_routing_setting(self, company_id: int | None = None, **overrides) -> int:
        if company_id is None:
            ...
            company = self.conn.execute('SELECT country_id, server_id FROM calling_companies WHERE id = %s', (company_id,)).fetchone()
    ...
    def test_create_first_company_routing_setting_and_logs_event(self):
        ...
        row = self.conn.execute('SELECT * FROM company_routing_settings WHERE id = %s', (setting_id,)).fetchone()
        ...
        event = self.conn.execute("SELECT * FROM change_log WHERE entity_type = 'company_routing_setting' AND entity_id = %s", (setting_id,)).fetchone()
    ...
    def test_company_routing_validates_campaign_geo_server_and_autorotation_flag(self):
        ...
        row = self.conn.execute('SELECT routing_mode, has_autorotation FROM company_routing_settings WHERE id = %s', (setting_id,)).fetchone()
    ...
    def test_company_routing_mode_change_versions_history_and_logs(self):
        ...
        old_row = self.conn.execute('SELECT * FROM company_routing_settings WHERE id = %s', (old_id,)).fetchone()
        new_row = self.conn.execute('SELECT * FROM company_routing_settings WHERE id = %s', (new_id,)).fetchone()
        ...
        event = self.conn.execute("SELECT * FROM change_log WHERE entity_type = 'company_routing_setting' AND entity_id = %s", (new_id,)).fetchone()
    ...
    def test_company_routing_comment_only_update_does_not_create_new_version_but_logs(self):
        ...
        returned_id = self.repo.update_company_routing_setting(setting_id=setting_id, country_id=self.country_id, server_id=self.conn.execute('SELECT server_id FROM company_routing_settings WHERE id = %s', (setting_id,)).fetchone()["server_id"], route_id=None, routing_mode="server_priority", has_autorotation=False, comment="after", updated_by=self.admin_id)
        ...
        event = self.conn.execute("SELECT * FROM change_log WHERE entity_type = 'company_routing_setting' AND entity_id = %s ORDER BY id DESC", (setting_id,)).fetchone()
...
class RoutingEventsRepositoryTest(unittest.TestCase):
    ...
    def test_none_scope_summary_contains_provider_and_route_without_server_or_campaign(self):
        ...
        log = self.conn.execute("SELECT summary FROM change_log WHERE entity_type = 'routing_event' AND entity_id = %s", (event_id,)).fetchone()[0]
    ...
    def test_server_priority_creates_priority_when_missing(self):
        ...
        self.assertEqual(self.conn.execute('SELECT old_route_id FROM routing_events WHERE id = %s', (event_id,)).fetchone()[0], None)
    ...
    def test_server_priority_multiple_server_ids_create_one_event_and_many_application_rows(self):
        ...
        self.conn.execute('INSERT INTO server_route_priorities(country_id, server_id, current_route_id, previous_route_id, changed_by, created_by) VALUES (%s, %s, %s, NULL, %s, %s)', (self.country_id, self.server_id, self.route_id, self.admin_id, self.admin_id))
        self.conn.execute('INSERT INTO server_route_priorities(country_id, server_id, current_route_id, previous_route_id, changed_by, created_by) VALUES (%s, %s, %s, NULL, %s, %s)', (self.country_id, server_2, self.route_id, self.admin_id, self.admin_id))
        self.conn.commit()
        ...
        self.assertEqual(self.conn.execute('SELECT COUNT(*) FROM routing_events WHERE id = %s', (event_id,)).fetchone()[0], 1)
        rows = self.conn.execute('SELECT * FROM routing_event_servers WHERE routing_event_id = %s ORDER BY server_id', (event_id,)).fetchall()
        self.assertEqual(len(rows), 2)
        self.assertTrue(all(row["status"] == "applied" for row in rows))
        priorities = self.conn.execute('SELECT * FROM server_route_priorities WHERE country_id = %s ORDER BY server_id', (self.country_id,)).fetchall()
    ...
    def test_server_priority_multi_server_creates_missing_priority_with_null_previous(self):
        ...
        priority = self.conn.execute('SELECT * FROM server_route_priorities WHERE country_id = %s AND server_id = %s', (self.country_id, server_2)).fetchone()
        self.assertIsNone(priority["previous_route_id"])
    ...
    def test_server_priority_full_noop_does_not_create_event(self):
        ...
        for sid in (self.server_id, server_2):
            self.conn.execute('INSERT INTO server_route_priorities(country_id, server_id, current_route_id, previous_route_id, changed_by, created_by) VALUES (%s, %s, %s, NULL, %s, %s)', (self.country_id, sid, self.route_id, self.admin_id, self.admin_id))
        self.conn.commit()
    ...
    def test_server_priority_partial_noop_records_applied_and_skipped(self):
        server_2 = self.repo.create_server("EU2")
        self.conn.execute('INSERT INTO server_route_priorities(country_id, server_id, current_route_id, previous_route_id, changed_by, created_by) VALUES (%s, %s, %s, NULL, %s, %s)', (self.country_id, self.server_id, self.route_id, self.admin_id, self.admin_id))
        self.conn.execute('INSERT INTO server_route_priorities(country_id, server_id, current_route_id, previous_route_id, changed_by, created_by) VALUES (%s, %s, %s, NULL, %s, %s)', (self.country_id, server_2, self.alt_route_id, self.admin_id, self.admin_id))
        self.conn.commit()
        ...
        changed = self.conn.execute('SELECT * FROM server_route_priorities WHERE country_id = %s AND server_id = %s', (self.country_id, self.server_id)).fetchone()
        skipped = self.conn.execute('SELECT * FROM server_route_priorities WHERE country_id = %s AND server_id = %s', (self.country_id, server_2)).fetchone()
    ...
    def test_server_priority_rejects_inactive_server(self):
        self.conn.execute('UPDATE servers SET is_active = FALSE WHERE id = %s', (self.server_id,))
        self.conn.commit()
    ...
    def test_campaign_setting_without_active_setting_uses_server_priority_defaults(self):
        ...
        setting = self.conn.execute('SELECT * FROM company_routing_settings WHERE calling_company_id = %s', (self.company_id,)).fetchone()
        self.assertIsNotNone(setting)
        self.assertEqual(setting["routing_mode"], "autorotation")
        self.assertEqual(setting["has_autorotation"], 1)
        self.assertIsNone(setting["route_id"])
    ...
    def test_campaign_setting_enable_autorotation_versions_existing_setting(self):
        ...
        active = self.conn.execute('SELECT * FROM company_routing_settings WHERE calling_company_id = %s AND is_active = TRUE AND valid_to IS NULL', (self.company_id,)).fetchone()
    ...
    def test_campaign_setting_duplicate_disable_autorotation_is_blocked(self):
        ...
        self.assertEqual(self.conn.execute("SELECT COUNT(*) FROM routing_events").fetchone()[0], 0)
        self.assertEqual(self.conn.execute('SELECT COUNT(*) FROM company_routing_settings WHERE calling_company_id = %s', (self.company_id,)).fetchone()[0], 0)
    ...
    def test_campaign_setting_set_campaign_route_rejects_same_active_route(self):
        ...
        with self.assertRaisesRegex(BusinessRuleError, "уже прописан"):
            self.create_event(apply_scope="campaign_setting", calling_company_id=self.company_id, company_change_type="set_campaign_route", provider_id=self.provider_id, new_company_route_id=self.route_id)
        self.assertEqual(self.conn.execute('SELECT COUNT(*) FROM company_routing_settings WHERE calling_company_id = %s', (self.company_id,)).fetchone()[0], 1)
    ...
    def test_deactivation_does_not_roll_back_server_priority(self):
        ...
        priority = self.conn.execute('SELECT * FROM server_route_priorities WHERE country_id = %s AND server_id = %s', (self.country_id, self.server_id)).fetchone()
        self.assertEqual(priority["current_route_id"], self.route_id)
        self.assertEqual(self.conn.execute('SELECT is_active FROM routing_events WHERE id = %s', (event_id,)).fetchone()[0], 0)
    ...
    def test_editing_campaign_setting_event_syncs_active_routing_comment_only(self):
        ...
        active_before = self.conn.execute(
            'SELECT * FROM company_routing_settings WHERE calling_company_id = %s AND is_active = TRUE AND valid_to IS NULL',
            (self.company_id,),
        ).fetchone()
...
class RepositoryHlrUsageConfigTest(unittest.TestCase):
    ...
    def test_upsert_hlr_daily_usage_updates_existing_day(self):
        ...
        rows = self.conn.execute('SELECT * FROM hlr_daily_usage WHERE usage_date = %s', ("2026-07-12",)).fetchall()
        self.assertEqual(len(rows), 1)
    ...
    def test_set_hlr_limit_override_updates_existing_value(self):
        ...
        rows = self.conn.execute('SELECT value FROM app_settings WHERE key = %s', ("hlr_daily_limit_override",)).fetchall()
        self.assertEqual(len(rows), 1)
