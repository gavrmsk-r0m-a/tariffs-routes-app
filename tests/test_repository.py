...
class RepositoryBusinessRulesTest(unittest.TestCase):
    ...
    def test_manual_server_priority_route_change_moves_current_to_previous_and_logs_event(self):
        ...
        self.assertIn("Италия", event["summary"])
        self.assertIn("IT1", event["summary"])
        self.assertIn("Италия/Miatel/Pool_A@", event["summary"])
        self.assertIn("Италия/Sancom/RND@", event["summary"])
    ...
    def test_company_routing_validates_company_country_and_server_match(self):
        server_id = self.repo.create_server("routing-srv")
        company_id = self.create_company(server_id=server_id)
        other_country_id = self.repo.create_country("Португалия", "PT")
        other_server_id = self.repo.create_server("routing-srv-other")

        with self.assertRaisesRegex(BusinessRuleError, "GEO схемы маршрутизации"):
        ...
        with self.assertRaisesRegex(BusinessRuleError, "Сервер схемы маршрутизации"):
        ...
        setting_id = self.repo.create_company_routing_setting(calling_company_id=company_id, country_id=self.country_id, server_id=server_id, route_id=None, routing_mode="server_priority", has_autorotation=False, comment=None, created_by=self.admin_id)
    ...
    def test_company_routing_autorotation_requires_has_autorotation(self):
        server_id = self.repo.create_server("autorotation-required-srv")
        company_id = self.create_company(server_id=server_id, external_id="autorotation-required")
        with self.assertRaisesRegex(BusinessRuleError, "авторотации"):
    ...
    def test_company_routing_list_filters_by_company_external_id(self):
        first_server_id = self.repo.create_server("external-filter-srv-1")
        second_server_id = self.repo.create_server("external-filter-srv-2")
        first_company_id = self.create_company(server_id=first_server_id, external_id="1001")
        second_company_id = self.create_company(server_id=second_server_id, external_id="2002")
        self.repo.create_company_routing_setting(calling_company_id=first_company_id, country_id=self.country_id, server_id=first_server_id, route_id=None, routing_mode="server_priority", has_autorotation=False, comment="target", created_by=self.admin_id)
        self.repo.create_company_routing_setting(calling_company_id=second_company_id, country_id=self.country_id, server_id=second_server_id, route_id=None, routing_mode="mixed", has_autorotation=False, comment="other", created_by=self.admin_id)

        rows = self.repo.list_company_routing_settings({"company_id_external": "1001"})
        self.assertEqual(len(rows), 1)
        self.assertEqual(rows[0]["company_id_external"], "1001")
        self.assertEqual(rows[0]["comment"], "target")
        rows = self.repo.list_company_routing_settings({"company_id_external": "missing", "show_history": True})
    ...
    def test_company_routing_route_autorotation_country_and_server_changes_create_versions(self):
        ...
        self.conn.execute("UPDATE calling_companies SET country_id = ?, server_id = ? WHERE id = ?", (other_country_id, other_server_id, company_id))
