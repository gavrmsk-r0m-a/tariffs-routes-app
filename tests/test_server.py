...
class ServerSmokeTest(unittest.TestCase):
    ...
    def test_server_priority_manual_route_update_changes_current_and_previous(self):
        ...
        try:
            ...
            self.assertIn("Мексика", event["summary"])
            self.assertIn("EU1", event["summary"])
            self.assertIn("Мексика/Miatel/Pool_A@", event["summary"])
            self.assertIn("Мексика/Sancom/RND/0827pfx@", event["summary"])
        finally:
            conn.close()
        captured, content = self.request("/admin/server-priorities")
        self.assertEqual(captured["status"], "200 OK")
    ...
    def test_company_routing_setting_create_visible_and_filters_render(self):
        ...
        self.assertEqual(captured["status"], "303 See Other")
        captured, content = self.request("/admin/company-routing-settings?country_id=1&server_id=1&routing_mode=server_priority&is_active=1&company_id_external=1001")
        self.assertEqual(captured["status"], "200 OK")
