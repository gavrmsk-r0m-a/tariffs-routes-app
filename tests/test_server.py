...
class HlrUiStateScriptTest(unittest.TestCase):
    ...
    def test_hlr_debug_mode_bypasses_filters_and_renders_filtered_results(self):
        content = self._content()
        self.assertIn("function onFilterClick(filterType, value)", content)
        self.assertIn("state.filteredResults = state.rawResults;", content)
        ...
        self.assertIn("renderTable(state.filteredResults);", content)
        ...
        self.assertIn('console.log("RAW LENGTH", state.rawResults?.length);', content)
        self.assertIn('console.log("FILTERED LENGTH", state.filteredResults?.length);', content)
        self.assertIn('console.log("RAW SAMPLE ROW", state.rawResults?.[0]);', content)
        self.assertIn('console.log("COLUMN SCHEMA", state.columns);', content)
        self.assertIn("HLR DEBUG: filter click ignored", content)
        self.assertNotIn("state.rawResults.filter((row) => matchesStatus(row) && matchesType(row));", content)

    def test_hlr_counters_and_column_copy_use_expected_sources(self):
        content = self._content()
        self.assertIn("function countTokens(data)", content)
        self.assertIn("state.counters = { raw: countTokens(state.rawResults) };", content)
        self.assertIn("node.textContent = raw;", content)
        ...
        self.assertIn("if (filterDebug) filterDebug.textContent = 'Filtered: ' + data.length + ' / Raw: ' + state.rawResults.length;", content)
