...
class HlrUiStateScriptTest(unittest.TestCase):
    ...
    def test_hlr_filters_render_only_filtered_state_after_chip_click(self):
        ...
        self.assertIn("function onFilterClick(filterType, value)", content)
        self.assertIn("applyFilters(); renderTable(state.filteredResults);", content)
        self.assertIn("function renderTable(data)", content)
        self.assertIn("if (!data || !tbody) return;", content)
        self.assertIn("tbody.replaceChildren();", content)
        self.assertNotIn("renderTable(state.results)", content)
        self.assertNotIn("state.rawResults", content)
        self.assertNotIn("readJsonScript", content)
        self.assertNotIn("hlr-results-data", content)
        self.assertIn("const backendPayload =", content)
        self.assertIn("console.log(\"RAW:\", state.results.length);", content)
        self.assertIn("console.log(\"FILTERED:\", state.filteredResults.length);", content)
        self.assertIn("console.log(\"ACTIVE:\", JSON.parse(JSON.stringify(state.activeFilters)));", content)
        self.assertIn("state.filteredResults = state.results.filter((row) => matchesStatus(row) && matchesType(row));", content)
        self.assertIn("filters.forEach((button) => button.addEventListener('click', () => onFilterClick(button.dataset.filterType || 'status', button.dataset.hlrFilter || 'ALL')));", content)
        self.assertIn("rawChips.forEach((button) => button.addEventListener('click', () => onFilterClick('status', button.dataset.rawStatus || '')));", content)
        self.assertIn("typeChips.forEach((button) => button.addEventListener('click', () => onFilterClick('type', button.dataset.numberType || '')));", content)

    def test_hlr_counters_and_column_copy_use_expected_sources(self):
        content = self._content()
        self.assertIn("function countTokens(data)", content)
        self.assertIn("state.counters = { raw: countTokens(state.results), filtered: countTokens(data) };", content)
        ...
        self.assertIn("if (filterDebug) filterDebug.textContent = 'Filtered: ' + data.length + ' / Raw: ' + state.results.length;", content)
