...
class ServerSmokeTest(unittest.TestCase):
    ...
    def test_manual_phone_creation_without_provider_is_rejected(self):
        ...
        self.assertLess(content.index('<header class="phone-dialog-header">'), content.index('class="phone-dialog-error-slot"'))
        self.assertLess(content.index('class="phone-dialog-error-slot"'), content.index('<section class="phone-dialog-section"><h3>Основные параметры</h3>'))
