...
class HlrUiStateScriptTest(unittest.TestCase):
    ...
    def test_hlr_filters_render_only_filtered_state_after_chip_click(self):
        ...
        self.assertIn("console.log(\"RAW:\", state.rawResults.length);", content)
        ...
        self.assertIn("state.filteredResults = state.rawResults.filter((row) => matchesStatus(row) && matchesType(row));", content)
