...
class HlrUiStateScriptTest(unittest.TestCase):
    ...
    def test_hlr_filters_render_only_filtered_state_after_chip_click(self):
        ...
        self.assertIn("applyFilters(); renderTable();", content)
        self.assertIn("function renderTable()", content)
        self.assertIn("console.debug('[HLR filters] filteredResults length before render', state.filteredResults.length);", content)
