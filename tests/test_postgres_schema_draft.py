...
POSTGRES_SCHEMA = (
    REPO_ROOT / "docs" / "postgres" / "schema.postgres.sql"
)

RUNTIME_SQLITE_SCHEMA = (
    REPO_ROOT / "app" / "schema.sql"
)

LEGACY_SQLITE_SCHEMA = (
    REPO_ROOT / "scripts" / "legacy" / "schema.sqlite.sql"
)

OPTIONAL_CAMPAIGN_GEO_MIGRATION = (
    REPO_ROOT
    / "docs/postgres/migrations/0069l_optional_campaign_geo.sql"
)

NULLABLE_ROUTING_GEO_MIGRATION = (
    REPO_ROOT
    / "docs/postgres/migrations/0069m_nullable_routing_setting_geo.sql"
)

CHANGE_REASON_SCOPES_MIGRATION = (
    REPO_ROOT
...
class PostgresSchemaDraftTests(unittest.TestCase):
    def test_change_reason_scopes_schema_and_idempotent_migration_contract(self):
        ...
        self.assertIn(
            "uq_change_reason_scopes_reason_scope UNIQUE "
            "(reason_id, apply_scope)",
            schema,
        )
        self.assertIn(
            "to_regclass('change_reason_scopes')",
            migration,
        )
        self.assertIn(
        ...
        self.assertIn(
            "lower(reason.name) = lower(legacy.name)",
            migration,
        )
    ...
    def test_sqlite_schema_is_legacy_migration_only(self):
        self.assertFalse(RUNTIME_SQLITE_SCHEMA.exists())
        self.assertTrue(LEGACY_SQLITE_SCHEMA.exists())

        sql = LEGACY_SQLITE_SCHEMA.read_text(encoding="utf-8")
    ...
    def test_optional_campaign_geo_schema_and_migration_contract(self):
        schema = POSTGRES_SCHEMA.read_text(encoding="utf-8")
        migration = OPTIONAL_CAMPAIGN_GEO_MIGRATION.read_text(
            encoding="utf-8"
        )

        calling_company = schema.split(
            "CREATE TABLE IF NOT EXISTS calling_companies",
            1,
        )[1].split(");", 1)[0]

        routing_setting = schema.split(
        ...
        self.assertIn(
            ...
            "ux_calling_companies_multi_geo_identity",
            migration,
        )

        routing_migration = NULLABLE_ROUTING_GEO_MIGRATION.read_text(
            encoding="utf-8"
        )
