...
class RepositoryPostgresCoverageAuditTests(unittest.TestCase):
    ...
    def test_actual_repository_baseline(self):
        ...
        self.assertEqual(125, s['repository_public_methods_count']); self.assertEqual(71, s['smoke_covered_read_count']); self.assertEqual(0, s['deferred_read_only_count'])
        self.assertEqual(53, s['write_or_mutating_count']); self.assertEqual(1, s['infrastructure_or_mixed_count']); self.assertEqual(100.0, s['read_surface_coverage_percent'])
