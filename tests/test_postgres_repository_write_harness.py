...
class FakeRepo:
    ...
    def get_or_create_country(self, name, **kwargs):
        self.calls.append(("get_or_create_country", name, kwargs))
        if "country" not in self.conn.dictionary: self.conn.dictionary["country"] = {"id": 155, "is_active": True}
        return self.conn.dictionary["country"]["id"]
    def get_or_create_currency(self, code, **kwargs):
        self.calls.append(("get_or_create_currency", code, kwargs))
        if "currency" not in self.conn.dictionary: self.conn.dictionary["currency"] = {"id": 156, "is_active": True}
        return self.conn.dictionary["currency"]["id"]
    def get_or_create_provider(self, name, currency_id=None, **kwargs):
        self.calls.append(("get_or_create_provider", name, currency_id, kwargs))
        if "provider" not in self.conn.dictionary: self.conn.dictionary["provider"] = {"id": 157, "normalized_name": harness.normalize_provider_name(name), "default_currency_id": currency_id, "is_active": True}
        return self.conn.dictionary["provider"]["id"]
    def get_or_create_prefix(self, provider_id, prefix, **kwargs):
        self.calls.append(("get_or_create_prefix", provider_id, prefix, kwargs))
    ...
    def ensure_project_exists(self, name, **kwargs):
        ...
        self.conn.dictionary["project"] = {"name": name, "is_active": True}; return 1
    def ensure_phone_number_type_exists(self, name, **kwargs):
        self.calls.append(("ensure_phone_number_type_exists", name, kwargs))
        if "phone_type" in self.conn.dictionary: return 0
        self.conn.dictionary["phone_type"] = {"name": name, "is_active": True}; return 1
    def ensure_phone_assignment_type_exists(self, code, name=None, **kwargs):
        self.calls.append(("ensure_phone_assignment_type_exists", code, name, kwargs))
        if "assignment" in self.conn.dictionary: return 0
        self.conn.dictionary["assignment"] = {"code": code, "name": name if name is not None else code, "is_active": True}; return 1



class WriteHarnessTest(unittest.TestCase):
    def test_stage66e_currency_rate_probe_is_rollback_only(self):
        source = SCRIPT.read_text(encoding="utf-8")
        self.assertIn("def run_currency_rate_lifecycle_probe", source)
        for method in ("create_currency_rate(", "recalculate_current_tariffs_for_currency_rate(", "log_currency_rate_change("):
    ...
    def test_stage66c_route_phone_probe_is_caller_owned_and_rollback_only(self):
        ...
        self.assertIn("route_phone_link_lifecycle_probe", harness.empty_summary("postgresql://u:p@h/db")["probes"])
        self.assertIn("SAVEPOINT stage66c_", source)
        self.assertIn("ROLLBACK TO SAVEPOINT stage66c_", source)
        self.assertIn("repo.add_phone_to_route(", source)
        self.assertIn("repo.add_phone_to_route_by_number(", source)
        self.assertIn("repo.remove_phone_links_from_route(", source)
        self.assertGreaterEqual(source.count("commit=False"), 6)
        self.assertNotIn("conn.commit()", source)
        self.assertEqual(25, len(harness.empty_summary("postgresql://u:p@h/db")["probes"]))
    ...
    def test_stage65a_probe_is_rollback_only_and_savepoint_contained(self):
        source = SCRIPT.read_text(encoding="utf-8")
        self.assertIn('def run_company_routing_setting_lifecycle_probe', source)
        self.assertIn('create_company_routing_setting(', source)
        self.assertIn('update_company_routing_setting(', source)
        self.assertIn('deactivate_company_routing_setting(', source)
        self.assertGreaterEqual(source.count('commit=False'), 3)
        self.assertIn('SAVEPOINT stage65a_', source)
        ...
        self.assertEqual(len(harness.empty_summary("postgresql://u:p@h/db")["probes"]), 25)

    def test_stage66a_route_probe_is_rollback_only_and_savepoint_contained(self):
        source = SCRIPT.read_text(encoding="utf-8")
        self.assertIn("def run_route_import_lifecycle_probe", source)
        for method in ("create_route(", "update_route(", "update_route_import_fields("):
            self.assertIn(method, source)
    ...
    def test_stage65b_campaign_probe_covers_paths_validations_and_cleanup(self):
        source = SCRIPT.read_text(encoding="utf-8")
        self.assertIn("def run_routing_event_create_campaign_probe", source)
        for marker in ("set_campaign_route", "disable_autorotation", "remove_campaign_route"):
            self.assertIn(marker, source)
        for name in ("required", "missing_company", "invalid_type", "already_enabled",
                     "already_disabled", "missing_route", "wrong_geo", "same_route"):
            self.assertIn(name, source)
        self.assertIn("SAVEPOINT stage65b_", source)
        self.assertIn("ROLLBACK TO SAVEPOINT stage65b_", source)
        self.assertIn("commit=False", source)
        self.assertNotIn("conn.commit()", source)
        self.assertIn("routing_event_create_campaign_probe", harness.empty_summary("postgresql://u:p@h/db")["probes"])
    ...
    def test_stage55_dictionary_get_or_create_probe_is_rollback_only(self):
        conn, repo = FakeConnection(), FakeRepo(None); repo.conn = conn
        harness.run_dictionary_get_or_create_probe(repo, conn)
        calls = [call for call in repo.calls if call[0].startswith("get_or_create_")]
        self.assertEqual([call[0] for call in calls], ["get_or_create_country", "get_or_create_country", "get_or_create_currency", "get_or_create_currency", "get_or_create_provider", "get_or_create_provider", "get_or_create_prefix", "get_or_create_prefix", "get_or_create_prefix"])
        self.assertTrue(all(call[-1]["commit"] is False for call in calls))
        self.assertEqual(conn.dictionary, {})
        self.assertEqual(conn.commits, 0)
        self.assertGreaterEqual(conn.rollbacks, 3)

    def test_stage55_dictionary_get_or_create_probe_rolls_back_on_failure(self):
        conn, repo = FakeConnection(), FakeRepo(None); repo.conn = conn
        def fail(*args, **kwargs):
            raise RuntimeError("Stage 55 probe failure")
        repo.get_or_create_currency = fail
        with self.assertRaisesRegex(RuntimeError, "Stage 55 probe failure"):
            harness.run_dictionary_get_or_create_probe(repo, conn)
        self.assertGreaterEqual(conn.rollbacks, 2)
        self.assertEqual(conn.commits, 0)

    def test_stage56_dictionary_ensure_probe_is_rollback_only_and_exercises_ignore_paths(self):
        conn, repo = FakeConnection(), FakeRepo(None); repo.conn = conn
        harness.run_dictionary_ensure_probe(repo, conn)
        calls = [call for call in repo.calls if call[0].startswith("ensure_")]
        self.assertEqual([call[0] for call in calls], ["ensure_project_exists", "ensure_project_exists", "ensure_phone_number_type_exists", "ensure_phone_number_type_exists", "ensure_phone_assignment_type_exists", "ensure_phone_assignment_type_exists"])
        self.assertTrue(all(call[-1]["commit"] is False for call in calls))
        self.assertEqual(conn.dictionary, {})
        self.assertEqual(conn.commits, 0)
        self.assertGreaterEqual(conn.rollbacks, 3)

    def test_stage56_dictionary_ensure_probe_rolls_back_on_failure(self):
        conn, repo = FakeConnection(), FakeRepo(None); repo.conn = conn
        repo.ensure_phone_number_type_exists = lambda *args, **kwargs: (_ for _ in ()).throw(RuntimeError("Stage 56 probe failure"))
        with self.assertRaisesRegex(RuntimeError, "Stage 56 probe failure"):
            harness.run_dictionary_ensure_probe(repo, conn)
        self.assertGreaterEqual(conn.rollbacks, 2)
        self.assertEqual(conn.commits, 0)
    ...
    def test_json_and_output_with_fake_driver(self):
        ...
        with tempfile.TemporaryDirectory() as directory, patch.dict("sys.modules", {"psycopg": Driver, "psycopg.rows": types.SimpleNamespace(dict_row=dict)}), patch.object(harness, "Repository", lambda conn, backend: FakeRepo(conn)), patch.object(harness, "run_dictionary_snapshot_probe") as snapshot_probe, patch.object(harness, "run_provider_change_priority_probe") as priority_probe, patch.object(harness, "run_provider_change_create_probe") as create_probe, patch.object(harness, "run_routing_event_deactivate_probe") as routing_probe, patch.object(harness, "run_routing_event_update_probe") as update_probe, patch.object(harness, "run_routing_event_create_core_probe") as create_event_probe, patch.object(harness, "run_company_routing_setting_lifecycle_probe") as company_probe, patch.object(harness, "run_routing_event_create_campaign_probe") as campaign_probe, patch.object(harness, "run_route_import_lifecycle_probe") as route_probe, patch.object(harness, "run_phone_import_lifecycle_probe"), patch.object(harness, "run_route_phone_link_lifecycle_probe") as phone_probe, patch.object(harness, "run_tariff_lifecycle_probe") as tariff_probe, patch.object(harness, "run_currency_rate_lifecycle_probe") as currency_probe, patch.object(harness, "run_calling_company_tail_lifecycle_probe") as company_tail_probe:
            ...
            priority_probe.assert_called_once()
    ...
    def test_repository_uses_postgres_backend(self):
        ...
        with patch.dict("sys.modules", {"psycopg": Driver, "psycopg.rows": types.SimpleNamespace(dict_row=dict)}), patch.object(harness, "Repository", CapturingRepo), patch.object(harness, "run_dictionary_snapshot_probe"), patch.object(harness, "run_provider_change_priority_probe"), patch.object(harness, "run_provider_change_create_probe"), patch.object(harness, "run_routing_event_deactivate_probe"), patch.object(harness, "run_routing_event_update_probe"), patch.object(harness, "run_routing_event_create_core_probe"), patch.object(harness, "run_company_routing_setting_lifecycle_probe"), patch.object(harness, "run_routing_event_create_campaign_probe"), patch.object(harness, "run_route_import_lifecycle_probe"), patch.object(harness, "run_phone_import_lifecycle_probe"), patch.object(harness, "run_route_phone_link_lifecycle_probe"), patch.object(harness, "run_tariff_lifecycle_probe"), patch.object(harness, "run_currency_rate_lifecycle_probe"), patch.object(harness, "run_calling_company_tail_lifecycle_probe"):
